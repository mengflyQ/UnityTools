...
namespace CAE.Core
{
    ...
    public abstract class Unit : XObject, IUObjectDetacher
    {
        ...
        // custom property
        private Dictionary<string, CustomProperty> mCustomPropertyHash = new Dictionary<string, CustomProperty>();
        ...
        public Dictionary<string, CustomProperty> CustomPropertyHash
        { get { return mCustomPropertyHash; } }
        ...
        public virtual void SetOrientation(float yaw, bool degree)
        {
            mYaw = degree ? yaw : yaw * Mathf.Rad2Deg;
            Vector3 euler = UUnit.transform.eulerAngles;
            euler.y = mYaw;
            UUnit.transform.eulerAngles = euler;
        }
        public virtual void SetOrientation(Vector3 dir)
        {
            dir.y = 0f;
            if (dir.Equals(Vector3.zero))
                return;

            UUnit.transform.rotation = Quaternion.LookRotation(dir);
            mYaw = UUnit.transform.eulerAngles.y;
        }
        public virtual void SetOrientation(Quaternion qat)
        {
            UUnit.transform.rotation = qat;
            mYaw = UUnit.transform.eulerAngles.y;
        }
        ...
        protected void Init(Vector3 pos, float yaw, ECampType campType)
        {
            ...
            mOnGround = false;
            mOnTouchSide = false;
            mYaw = mUObject.transform.eulerAngles.y;
            mCamp = campType;

            SetBornPosition(pos);
            SetOrientation(yaw, true);
