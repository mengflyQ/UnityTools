...
namespace CAE.Core
{
    ...
    public sealed class ActionStatus
    {
        ...
        void ProcessAnimSpeed(ref int deltaTime)
        {
            ...
            {
                if (mActiveAction.ActionStatus == EActionState.Attack && mOwner.GetAttribute(Unit.EAttributeType.EAT_AttackSpeed) > 0)
                {
                    float speed = Mathf.Clamp((float)mOwner.GetAttribute(Unit.EAttributeType.EAT_AttackSpeed) * 0.001f, 0f, float.MaxValue);
                    deltaTime = (int)(deltaTime * speed);
                    mOwner.UUnit.SetAnimationSpeed(speed);
                }
                else if (mActiveAction.ActionStatus == EActionState.Move && mOwner.GetAttribute(Unit.EAttributeType.EAT_CurMoveSpeed) != (mOwner.GetAttribute(Unit.EAttributeType.EAT_MoveSpeed) * 0.001f))
                {
                    float speed = (float)(mOwner.GetAttribute(Unit.EAttributeType.EAT_CurMoveSpeed) / (mOwner.GetAttribute(Unit.EAttributeType.EAT_MoveSpeed) * 0.001f));
        ...
        private void ProcessOrientation()
        {
            if (mActiveAction.FaceTarget)
            {
                ...
                {
                    mOwner.SetOrientation(mOwner.Target.Position - mOwner.Position);
                }
                else
                {
                    ...
                    mOwner.SetOrientation(mFaceDirection);
                }
            }
        ...
        private void Reset()
        {
            ...
            {
                if (this.mActiveAction.InterruptList[index].Enabled)
        ...
        private bool OnTriggerEvent(Event evt, int deltaTime)
        {
            return true;
        }

        private void TriggerEvent(Event evt, int deltaTime)
        {
        ...
        private void ProcessEventList(int deltaTime)
        {
            ...
            {
                Event evt = mActiveAction.EventList[mCurEventIndex];
            ...
            for (int i = 0; i < mCurEventIndex; ++i)
            {
                Event evt = mActiveAction.EventList[i];
                if (evt.EventData is IEventEveryTimeExcute)
                {
                    IEventEveryTimeExcute executer = evt.EventData as IEventEveryTimeExcute;
                    executer.ExecuteEveryTime(mOwner);
                }
            }
        }
        ...
        private void ProcessEventWhenActionEnd()
        {
            ...
            {
                Event evt = mActiveAction.EventList[mCurEventIndex];
                if (evt.EventData is IEventWhenActionEndExecute)
                {
                    IEventWhenActionEndExecute executer = evt.EventData as IEventWhenActionEndExecute;
                    executer.ExecuteOnActionEnd(mOwner);
                }
            }
        } 
        ...
        private void ProcessAttackDefList(int deltaTime)
        {
            ...
            {
                ActionAttackDef atk = mActiveAction.AttackList[mCurAttackHitIndex];
                if (atk.TriggerTime < mCurActionTime)
                {
        ...
        {
            int idx = 0;
            using (List<ActionInterrupt>.Enumerator itr = mActiveAction.InterruptList.GetEnumerator())
            {
                while (itr.MoveNext())
                {
                    if (mCurActionTime >= itr.Current.InterruptTime)
                    {
                    ...
                    if (GetInterruptEnabled(idx++) && ProcessActionInterrupt(itr.Current))
                        return true;
                }
            }
        ...
        private void ProcessMoving(int deltaTime)
        {
            ...
            mOwner.CustomPropertyHash[CustomProperty.sVelocityY].Value = mVelocity.y;
        }
        ...
        {
            mOwner.OnCommand(Unit.ECommandType.ECT_ActionStart, action);
        }

        private void OnChangingAction(Action oldAction, Action newAction, bool interrupt)
        {
            mOwner.OnCommand(Unit.ECommandType.ECT_ActionChanging, oldAction, newAction, interrupt);
        }

        private void OnActionEnd(Action action, bool interrupt)
        {
            mOwner.OnCommand(Unit.ECommandType.ECT_ActionEnd, action, interrupt);
        }

        private void OnActionFinish(Action action)
        {
            mOwner.OnCommand(Unit.ECommandType.ECT_ActionFinish, action);
        }

        public void ChangeAction(string id, int deltaTime = 0, bool interrupt = false)
        {
