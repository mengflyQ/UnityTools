    ...
    public sealed class StatePlay : IGameState
    {
        ...
        private void onSceneLoaded()
        {
            switch (mSceneName)
            {
                case "rpg":
                    {
                        Unit p = UnitMgr.Instance.CreateUnit(Unit.EUnitType.EUT_LocalPlayer, "1", Vector3.zero, 0f, ECampType.EFT_Friend);
                        p.UseWeapon("WQ01001");
                ...
                case "act":
                    {
                        Unit p = UnitMgr.Instance.CreateUnit(Unit.EUnitType.EUT_LocalPlayer, "2", Vector3.zero, 0f, ECampType.EFT_Friend);

                        UnitMgr.Instance.CreateUnit(Unit.EUnitType.EUT_Monster, "2", new Vector3(0, 0, 12f), 0, ECampType.EFT_Enemy);
