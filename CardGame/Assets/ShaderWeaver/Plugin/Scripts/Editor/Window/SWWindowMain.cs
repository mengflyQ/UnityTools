...
//			//performance impact
		...
		protected bool NodesOk()
		{
			foreach (var item in nodes) {
				...
				if (item is SWNodeCode) {
					if (item.data.children.Count < item.data.childPortNumber) {
						EditorUtility.DisplayDialog ("Before Save", string.Format ("Please assign all {0}'s children.", item.data.name), "OK");
						return false;
					}
				}
			}
...
//			}
		...
		void BreakConnections(Dictionary<string,SWNodeBase> tempNodeAll,Vector2 pos)
		{
			foreach (var node in nodes) {
				...
				for (int i = 0; i < node.rectRights.Count; i++) {
					...
					if (rect.Contains (pos)) {
						SWUndo.Record (node);
						foreach (var ii in node.data.parent) {
							SWUndo.Record (tempNodeAll[ii]);
						}
		...
		void NodeOperation()
		{
			...
			if (((Event.current.alt||Event.current.control) && SWCommon.GetMouseDown (0,false))
				||SWCommon.GetMouseDown (2,false)) 
			{
				BreakConnections (tempNodeAll,Event.current.mousePosition);
				return;
			}
			...
			if (selectMode == SelectMode.move) {
				if (SWCommon.GetMouse (0) && InMap() ) {
					...
					foreach (var item in selection) {
						tempNodeAll[item].data.rect.center += v;
					}
					RepaintGetDirty();
				}
		...
		void ResetPos(List<SWDataNode> datas)
		{
			var center = Vector2.zero;
			for (int i = 0; i < datas.Count; i++) {
				center += datas [i].rect.center;
			}
			center /= datas.Count;


			for (int i = 0; i < datas.Count; i++) {
				datas [i].rect.center = (datas [i].rect.center - center) + mousePos;
			}
		}
...
//				}
		...
		#region RMBMenu
		protected override void RMBMenu_Init ()
		{
			base.RMBMenu_Init ();
			RMBMenu_List.Add(new RMBItem("Copy",OnPress_RMBMenu_Copy,null));
			RMBMenu_List.Add(new RMBItem("Paste",OnPress_RMBMenu_Paste,null));
			RMBMenu_List.Add(new RMBItem("Rename",OnPress_RMBMenu_Rename,null));
			RMBMenu_List.Add(new RMBItem("Duplicate",OnPress_RMBMenu_Duplicate,null));
			RMBMenu_List.Add(new RMBItem("Delete",OnPress_RMBMenu_Delete,null));
			RMBMenu_List.Add(new RMBItem("BreakConn",OnPress_RMBMenu_BreakConnections,null));
		}

		protected void OnPress_RMBMenu_Copy()
		{
			Copy ();
		}
		protected void OnPress_RMBMenu_Paste()
		{
			Paste ();
		}
		protected void OnPress_RMBMenu_Rename()
		{
			if (selection.Count > 0) {
				var tempNodeAll = NodeAll ();
				tempNodeAll [selection [0]].StartTitleEditing ();
				repaintDirty = true;
			}
		}
		protected void OnPress_RMBMenu_Duplicate()
		{
			Copy ();
			Paste ();
		}
		protected void OnPress_RMBMenu_Delete()
		{
			KeyCmd_Delete ();
		}
		protected void OnPress_RMBMenu_BreakConnections()
		{
			var tempNodeAll = NodeAll ();
			BreakConnections (tempNodeAll,RMBMenu_Pos);
		}

		protected override void RMBMenu_Start ()
		{
			base.RMBMenu_Start ();
			Vector2 mp = Event.current.mousePosition;
			KeyCmd_SelectClean ();
		...
		protected override void RMBMenu_Clean ()
		{
			base.RMBMenu_Clean ();
			KeyCmd_SelectClean ();
		}
