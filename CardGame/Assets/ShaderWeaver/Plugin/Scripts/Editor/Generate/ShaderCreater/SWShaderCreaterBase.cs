...
//				StringAddLine( string.Format("Shader \"{0}\"{{   ",window.data.sn));
		...
		protected virtual void PropertyField()
		{
			StringAddLine (string.Format("\t\t{0}_Color (\"Color\", Color) = ({1},{2},{3},{4})",
				...
				window.nRoot.data.effectDataColor.color.r.ToStringEX(),
				window.nRoot.data.effectDataColor.color.g.ToStringEX(),
				window.nRoot.data.effectDataColor.color.b.ToStringEX(),
				window.nRoot.data.effectDataColor.color.a.ToStringEX()
            ));
			...
			foreach (var item in window.data.paramList) {
				if(item.type == SWParamType.FLOAT)
					StringAddLine( string.Format("\t\t{0} (\"{0}\", Float) = {1}",item.name,item.defaultValue.ToStringEX()));
				if(item.type == SWParamType.RANGE)
					StringAddLine( string.Format("\t\t{0} (\"{0}\", Range({1},{2})) = {3}",item.name,item.min.ToStringEX(), item.max.ToStringEX(), item.defaultValue.ToStringEX()));
			}
		...
		public void ProcessAll(  SWNodeBase root)
		{
			...
			StringAddLine( string.Format("\t\t\t\tclip(result.a - {0});",
				window.data.clipValue.ToStringEX()));
