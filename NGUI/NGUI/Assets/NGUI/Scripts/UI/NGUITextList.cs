...
namespace NGUI
{
    ...
    /// <summary>
    /// Text list can be used with a NGUILabel to create a scrollable multi-line text field that's
    /// easy to add new entries to. Optimal use: chat window.
    /// </summary>

    [AddComponentMenu("NGUI/UI/NGUITextList")]
	...
	public bool isValid { get { return textLabel != null && textLabel.bitmapFont != null; } }
        ...
        {
            get { return (textLabel != null) ? textLabel.fontSize + textLabel.effectiveSpacingY : 20f; }
        }
        ...
        void Start()
        {
            ...
            textLabel.overflowMethod = NGUILabel.Overflow.ClampContent;

            if (style == Style.Chat)
            {
                textLabel.pivot = NGUIWidget.Pivot.BottomLeft;
                scrollValue = 1f;
            }
            else
            {
                textLabel.pivot = NGUIWidget.Pivot.TopLeft;
                scrollValue = 0f;
            }
        }
        ...
        protected void Rebuild()
        {
            if (isValid)
            {
                ...
                // Although we could simply use NGUILabel.Wrap, it would mean setting the same data
