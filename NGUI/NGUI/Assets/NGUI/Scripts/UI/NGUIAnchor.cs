namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/UI/NGUIAnchor")]
	public class NGUIAnchor : MonoBehaviour
	{
		...
		/// <summary>
		/// If set to 'true', NGUIAnchor will execute once, then will be disabled.
		...
		/// while "-10" in x is 10 pixels to the left based on the pixel values set in NGUIRoot.
		/// </summary>

		public Vector2 pixelOffset = Vector2.zero;

		// Deprecated legacy functionality
		[HideInInspector] [SerializeField] NGUIWidget widgetContainer;
...
#endif
			NGUICamera.onScreenResize += ScreenSizeChanged;
		}
		...
		void Update()
		{
			...
			NGUIWidget wc = (container == null) ? null : container.GetComponent<NGUIWidget>();
			NGUIPanel  pc = (container == null && wc == null) ? null : container.GetComponent<NGUIPanel>();
			...
			{
				if (pc.clipping == NGUIDrawCall.Clipping.None)
				{
			...
			else if (container != null)
			{
				...
				Bounds b = (root != null)
					           ? NGUIMath.CalculateRelativeWidgetBounds(root, container.transform)
