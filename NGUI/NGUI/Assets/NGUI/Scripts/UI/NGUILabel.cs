namespace NGUI
{
    ...
    [ExecuteInEditMode]
    [AddComponentMenu("NGUI/UI/NGUILabel")]
    public class NGUILabel : NGUIWidget
    {
        [DoNotObfuscateNGUI]
        public enum Effect
        {
        ...
        [System.NonSerialized] public static BetterList<Vector3> vertsForOnFill = new BetterList<Vector3>();
        [System.NonSerialized] public static BetterList<Vector2> uvsForOnFill = new BetterList<Vector2>();
        [System.NonSerialized] public static BetterList<Color32> colsForOnFill = new BetterList<Color32>();
        /// <summary>
        ...
        [System.Obsolete("Use 'overflowMethod == NGUILabel.Overflow.ShrinkContent' instead")]
        ...
        static BetterList<NGUILabel> mList = new BetterList<NGUILabel>();
        ...
        static void OnFontChanged(Font font)
        {
                ...
                if (lbl != null)
                {
                    ...
                    if (fnt == font)
                    {
                        ...
                        if (mTempDrawcalls == null)
                            mTempDrawcalls = new BetterList<NGUIDrawCall>();
            ...
            for (int i = 0; i < mList.size; ++i)
            {
                ...
                if (lbl != null)v [kp;' 5rfc328i0u
                {
                    ...
                    if (fnt == font)
                    {
                        if (mTempLabels == null) mTempLabels = new BetterList<NGUILabel>();
            ...
            if (mFontChangedDepth == 1 && mTempLabels != null)
            {
                ...
                for (int i = 0, imax = mTempLabels.size; i < imax; ++i)
                {
                    ...
                    if (dc != null)
                    {
                        if (mTempDrawcalls == null) mTempDrawcalls = new BetterList<NGUIDrawCall>();
        ...
        [System.NonSerialized] static BetterList<NGUIDrawCall> mTempDrawcalls;
        //[System.NonSerialized] static BetterList<NGUILabel> mTempLabels;
        ...
        /// <summary>
        /// NGUILabel needs additional processing when something changes.
        /// </summary>
        ...
        //[System.Obsolete("Use NGUILabel.GetCharacterAtPosition instead")]
	...
	int IndexOfAny (string input, char[] any, int start)
	{
        ...
        public int GetCharacterIndex(int currentIndex, KeyCode key)
        {
            if (isValid)
            {
                ...
                if (mTempVerts.Size > 0)
                {
                    ...
                    for (int i = 0, imax = mTempIndices.Size; i < imax; ++i)
                    {
                        if (mTempIndices.Buffer[i] == currentIndex)
                        {
                            // Determine position on the line above or below this character
                            Vector2 localPos = mTempVerts.Buffer[i];
        ...
        public void PrintOverlay(int start, int end, NGUIGeometry caret, NGUIGeometry highlight, Color caretColor, Color highlightColor)
        {
            ...
            if (highlight != null && start != end)
            {
                ...
                if (highlight.verts.Size > startingVertices)
                {
                    ...
                    for (int i = startingVertices, imax = highlight.verts.Size; i < imax; ++i)
                    {
            ...
            for (int i = startingCaretVerts, imax = caret.verts.Size; i < imax; ++i)
            {
        ...
        public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
        {
            ...
            verts.AddRange(vertsForOnFill, vertsForOnFill.Size);
            uvs.AddRange(uvsForOnFill, uvsForOnFill.Size);
            cols.AddRange(colsForOnFill, colsForOnFill.Size);
            ...
            if (NGUIText.symbolStyle == NGUIText.SymbolStyle.NoOutline)
                for (int i = 0, imax = cols.Size; i < imax; i++)
                    if (cols.Buffer[i].r == -1f) cols.Buffer[i] = Color.white;
        ...
        public Vector2 ApplyOffset(BetterList<Vector3> verts, int start)
        {
            ...
            for (int i = start, imax = verts.Size; i < imax; ++i)
            {
                v = verts.Buffer[i];
                v.x += fx;
                v.y += fy;
