namespace NGUI
{
	...
	public interface INGUIAtlas
	{
		...
		List<NGUISpriteData> spriteList { get; set; }
		...
		/// Most obvious use would be on retina screen displays. The resolution doubles, but with NGUIRoot staying the same
		...
		NGUISpriteData GetSprite(string name);
	...
	public class NGUIAtlas : ScriptableObject, INGUIAtlas
	{
		...
		[HideInInspector] [SerializeField] List<NGUISpriteData> mSprites = new List<NGUISpriteData>();
		...
		public List<NGUISpriteData> spriteList
		{
			get
			{
				var rep = replacement;
		...
		public NGUISpriteData GetSprite(string name)
		{
			var rep = replacement;
		...
		{
			mSprites.Sort(delegate(NGUISpriteData s1, NGUISpriteData s2) { return s1.name.CompareTo(s2.name); });
#if UNITY_EDITOR
		...
		public BetterList<string> GetListOfSprites(string match)
		{
			...
			for (int i = 0, imax = mSprites.Count; i < imax; ++i)
			{
				...
				if (s != null && !string.IsNullOrEmpty(s.name) &&
				    string.Equals(match, s.name, StringComparison.OrdinalIgnoreCase))
				{
					list.Add(s.name);
					return list;
				}
			...
			var keywords =
				match.Split(new char[] {' '}, StringSplitOptions.RemoveEmptyEntries);
		...
		public void MarkAsChanged()
		{
			...
			var list = NGUITools.FindActive<NGUISprite>();
