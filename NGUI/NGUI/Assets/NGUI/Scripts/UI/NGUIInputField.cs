...
#define MOBILE
	...
	[AddComponentMenu("NGUI/UI/NGUIInputField")]
	public class NGUIInputField : MonoBehaviour
	{
		[DoNotObfuscateNGUI]
	...
	[DoNotObfuscateNGUI] public enum KeyboardType
	{
		...
		static public NGUIInputField current;

		/// <summary>
		/// Currently selected input field, if any.
		/// </summary>

		static public NGUIInputField selection;

		/// <summary>
		/// Text label used to display the input's value.
		/// </summary>

		public NGUILabel label;
	...
	static bool mWaitForKeyboard = false;
		...
		[System.NonSerialized] protected NGUITexture mHighlight      = null;
		[System.NonSerialized] protected NGUITexture mCaret          = null;
		...
		{
			get { return hideInput && label != null && !label.multiLine && inputType != InputType.Password; }
		}

		[System.Obsolete("Use NGUIInputField.value instead")]
		public string text
		{
		...
		[System.Obsolete("Use NGUIInputField.isSelected instead")]
		...
		public bool isSelected
		{
			...
			set
			{
				if (!value)
				{
					if (isSelected) NGUICamera.selectedObject = null;
				}
				else NGUICamera.selectedObject = gameObject;
			}
		}
...
#if !MOBILE
		[System.NonSerialized] NGUIInputOnGUI mOnGUI;
#endif
		[System.NonSerialized] NGUICamera mCam;
...
#if !MOBILE
				if (mOnGUI == null)
					mOnGUI = gameObject.AddComponent<NGUIInputOnGUI>();
#endif
				OnSelectEvent();
			}
			else
...
#endif // MOBILE
				{
					Vector2 pos = (NGUICamera.current != null && NGUICamera.current.cachedCamera != null)
						              ? NGUICamera.current.cachedCamera.WorldToScreenPoint(label.worldCorners[0])
		...
		if (mKeyboard != null)
		{
			...
			// Cache the camera
			if (mCam == null) mCam = NGUICamera.FindCameraForLayer(gameObject.layer);
			...
			if (mCam != null)
			{
				...
				if (NGUICamera.GetKeyDown(mCam.submitKey0) ||
				    (mCam.submitKey0 == KeyCode.Return && NGUICamera.GetKeyDown(KeyCode.KeypadEnter)))
				{
					if (newLine)
					{
						Insert("\n");
					...
					{
						if (NGUICamera.controller.current != null)
							NGUICamera.controller.clickNotification = NGUICamera.ClickNotification.None;
						NGUICamera.currentKey = mCam.submitKey0;
						Submit();
					}
				}

				if (NGUICamera.GetKeyDown(mCam.submitKey1) ||
				    (mCam.submitKey1 == KeyCode.Return && NGUICamera.GetKeyDown(KeyCode.KeypadEnter)))
				{
					if (newLine)
					{
						Insert("\n");
					...
					else
					{
						if (NGUICamera.controller.current != null)
							NGUICamera.controller.clickNotification = NGUICamera.ClickNotification.None;
						NGUICamera.currentKey = mCam.submitKey1;
						Submit();
					}
				}

				if (!mCam.useKeyboard && NGUICamera.GetKeyUp(KeyCode.Tab))
					OnKey(KeyCode.Tab);
			}
		}
		...
		protected int GetCharUnderMouse()
		{
			...
			Ray       ray     = NGUICamera.currentRay;
			Plane     p       = new Plane(corners[0], corners[1], corners[2]);
			float     dist;
			return p.Raycast(ray, out dist)
				       ? mDrawStart + label.GetCharacterIndexAtPosition(ray.GetPoint(dist), false)
				       : 0;
		}
		...
		protected virtual void OnPress(bool isPressed)
		{
			if (isPressed && isSelected && label != null &&
			    (NGUICamera.currentScheme == NGUICamera.ControlScheme.Mouse ||
			     NGUICamera.currentScheme == NGUICamera.ControlScheme.Touch))
			{
		...
		public void UpdateLabel()
		{
			if (label != null)
			{
				...
				else
				{
					...
					if (selected && label.overflowMethod == NGUILabel.Overflow.ClampContent && label.maxLineCount == 1)
					{
			...
			if (selected && (mKeyboard == null || inputShouldBeHidden))
...
#endif
				{
					...
					if (start != end)
					{
						if (mHighlight == null)
						{
							mHighlight              = NGUITools.AddWidget<NGUITexture>(label.cachedGameObject);
					...
					if (mCaret == null)
					{
						mCaret              = NGUITools.AddWidget<NGUITexture>(label.cachedGameObject);
					...
					if (start != end)
					{
						label.PrintOverlay(start, end, mCaret.geometry, mHighlight.geometry, caretColor,
						                   selectionColor);
