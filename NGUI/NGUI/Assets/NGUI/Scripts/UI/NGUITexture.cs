namespace NGUI
{
    ...
    /// Keep in mind though that this will create an extra draw call with each NGUITexture present, so it's
    /// best to use it only for backgrounds or temporary visible widgets.
    /// </summary>

    [ExecuteInEditMode]
    [AddComponentMenu("NGUI/UI/NGUITexture")]
    public class NGUITexture : NGUIBasicSprite
    {
        ...
        /// <summary>
        /// Texture used by the NGUITexture. You can set it directly, without the need to specify a material.
        /// </summary>
        ...
        public override Vector4 drawingDimensions
        {
            get
            {
                ...
                if (mTexture != null && mType != NGUISprite.Type.Tiled)
                {
                    ...
                    if (w > 0 && h > 0 && (mType == NGUISprite.Type.Simple || mType == NGUISprite.Type.Filled))
                    {
                        if ((w & 1) != 0) ++padRight;
                        if ((h & 1) != 0) ++padTop;
                    ...
                    if (mFlip == NGUISprite.Flip.Horizontally || mFlip == NGUISprite.Flip.Both)
                    {
                        x0 += padRight * px;
                    }
                    else x1 -= padRight * px;

                    if (mFlip == NGUISprite.Flip.Vertically || mFlip == NGUISprite.Flip.Both)
                    {
                        y0 += padTop * py;
                    }
                    else y1 -= padTop * py;
        ...
        public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
        {
            ...
            Rect outer = new Rect(mRect.x * tex.width, mRect.y * tex.height, tex.width * mRect.width,
                                  tex.height * mRect.height);
            Rect inner = outer;
            Vector4 br = border;
            inner.xMin += br.x;
            inner.yMin += br.y;
