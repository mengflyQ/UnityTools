namespace NGUI
{
	...
	/// <summary>
	/// Simple script that lets you localize a NGUIWidget.
	/// </summary>

	[ExecuteInEditMode]
	[RequireComponent(typeof(NGUIWidget))]
	[AddComponentMenu("NGUI/UI/NGUILocalize")]
	public class NGUILocalize : MonoBehaviour
	{
			...
			set
			{
				if (!string.IsNullOrEmpty(value))
				{
					...
					if (lbl != null)
					{
						...
						NGUIInputField input = NGUITools.FindInParents<NGUIInputField>(lbl.gameObject);
		...
		void OnLocalize()
		{
			...
			{
				NGUILabel lbl          = GetComponent<NGUILabel>();
