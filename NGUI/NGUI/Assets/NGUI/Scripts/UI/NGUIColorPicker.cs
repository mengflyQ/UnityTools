namespace NGUI
{
	...
	/// <summary>
	/// Attach this script to a NGUITexture to turn it into a color picker.
	/// The color picking texture will be generated automatically.
	/// </summary>

	[RequireComponent(typeof(NGUITexture))]
	public class NGUIColorPicker : MonoBehaviour
	{
		static public NGUIColorPicker current;
		...
		public NGUIWidget selectionWidget;
		...
		[System.NonSerialized] NGUITexture mUITex;
		...
		{
			mTrans = transform;
			mUITex = GetComponent<NGUITexture>();
			mCam   = NGUICamera.FindCameraForLayer(gameObject.layer);
		...
		{
			if (enabled && pressed && NGUICamera.currentScheme != NGUICamera.ControlScheme.Controller) Sample();
		}
		...
		void Sample()
		{
			var pos     = mTrans.InverseTransformPoint(NGUICamera.lastWorldPosition);
