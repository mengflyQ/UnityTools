namespace NGUI
{
    ...
    public abstract class NGUIBasicSprite : NGUIWidget
    {
        [DoNotObfuscateNGUI]
        public enum Type
        {
        ...
        /// <summary>
        /// Trimmed space in the atlas around the sprite. X = left, Y = bottom, Z = right, W = top. Overridden in NGUISprite.
        /// </summary>
        ...
        protected void SimpleFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, ref Vector4 v,
                                  ref Vector4 u, ref Color c)
        {
        ...
        protected void SlicedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, ref Vector4 v,
                                  ref Vector4 u, ref Color gc)
        {
            ...
            if (geometry.verts.Buffer == null || geometry.verts.Buffer.Length < newSize)
            {
        ...
        void AddVertexColours(BetterList<Color32> cols, ref Color color, int x, int y)
        {
            Vector4 br = border * pixelSize;
        ...
        protected void TiledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, ref Vector4 v, ref Color c)
        {
            var tex = mainTexture;
            if (tex == null) return;
        ...
        protected void FilledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, ref Vector4 v,
                                  ref Vector4 u, ref Color c)
        {
            ...
            if (mFillAmount < 1f)
            {
                ...
                if (mFillDirection == FillDirection.Radial180)
                {
                    for (int side = 0; side < 2; ++side)
                    {
                        ...
                        if (RadialCut(mTempPos, mTempUVs, Mathf.Clamp01(val), !mInvert,
                                      NGUIMath.RepeatIndex(side + 3, 4)))
                        {
                            for (int i = 0; i < 4; ++i)
                            {
                                verts.Add(mTempPos[i]);
        ...
        protected void AdvancedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, ref Vector4 v,
                                    ref Vector4 u, ref Color c)
        {
            var tex = mainTexture;
            if (tex == null) return;
            ...
            for (int x = 0; x < 3; ++x)
            {
                ...
                for (int y = 0; y < 3; ++y)
                {
                    ...
                    if (x == 1 && y == 1) // Center
                    {
                        if (centerType == AdvancedType.Tiled)
                        {
                            ...
                            while (tileStartY < endPositionY)
                            {
                                ...
                                {
                                    textureEndY = Mathf.Lerp(textureStartY, textureEndY,
                                                             (endPositionY - tileStartY) / tileSize.y);
                                ...
                                while (tileStartX < endPositionX)
                                {
                                    ...
                                    {
                                        textureEndX = Mathf.Lerp(textureStartX, textureEndX,
                                                                 (endPositionX - tileStartX) / tileSize.x);
                    ...
                    else if (x == 1) // Top or bottom
                    {
                        ...
                        else if ((y == 0 && bottomType != AdvancedType.Invisible) ||
                                 (y == 2 && topType != AdvancedType.Invisible))
                        {
                            Fill(verts, uvs, cols,
                                 mTempPos[x].x, mTempPos[x2].x,
                                 mTempPos[y].y, mTempPos[y2].y,
                    ...
                    else if (y == 1) // Left or right
                    {
                        ...
                        else if ((x == 0 && leftType != AdvancedType.Invisible) ||
                                 (x == 2 && rightType != AdvancedType.Invisible))
                        {
                            Fill(verts, uvs, cols,
                                 mTempPos[x].x, mTempPos[x2].x,
                                 mTempPos[y].y, mTempPos[y2].y,
        ...
        static void Fill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols,
                         float v0x, float v1x, float v0y, float v1y, float u0x, float u1x,
                         float u0y, float u1y,
                         Color32 col)
        {
