namespace NGUI
{
	...
	static public class NGUIMath
	{
		...
		[System.Diagnostics.DebuggerHidden]
		[System.Diagnostics.DebuggerStepThrough]
		static void CalculateRelativeWidgetBounds(Transform     content, bool considerInactive,
		                                          bool          isRoot,
		                                          ref Matrix4x4 toLocal, ref Vector3 vMin, ref Vector3 vMax,
		                                          ref bool      isSet,   bool        considerChildren)
		{
			...
			NGUIPanel p = isRoot ? null : content.GetComponent<NGUIPanel>();
			...
			if (p != null && p.clipping != NGUIDrawCall.Clipping.None)
			{
			...
			else // No panel present
			{
				...
				NGUIWidget w = content.GetComponent<NGUIWidget>();
				...
				for (int i = 0, imax = content.childCount; i < imax; ++i)
					CalculateRelativeWidgetBounds(content.GetChild(i), considerInactive, false, ref toLocal, ref vMin,
					                              ref vMax,            ref isSet,        true);
			}
		}
		...
		static public Vector2 GetPivotOffset(NGUIWidget.Pivot pv)
		{
			Vector2 v = Vector2.zero;

			if (pv == NGUIWidget.Pivot.Top || pv == NGUIWidget.Pivot.Center || pv == NGUIWidget.Pivot.Bottom) v.x = 0.5f;
			else if (pv == NGUIWidget.Pivot.TopRight || pv == NGUIWidget.Pivot.Right ||
			         pv == NGUIWidget.Pivot.BottomRight) v.x = 1f;
			else v.x                                       = 0f;

			if (pv == NGUIWidget.Pivot.Left || pv == NGUIWidget.Pivot.Center      || pv == NGUIWidget.Pivot.Right) v.y = 0.5f;
			else if (pv == NGUIWidget.Pivot.TopLeft || pv == NGUIWidget.Pivot.Top || pv == NGUIWidget.Pivot.TopRight
			) v.y    = 1f;
			else v.y = 0f;
		...
		static public NGUIWidget.Pivot GetPivot(Vector2 offset)
		{
			if (offset.x == 0f)
			{
				if (offset.y == 0f) return NGUIWidget.Pivot.BottomLeft;
				if (offset.y == 1f) return NGUIWidget.Pivot.TopLeft;
				return NGUIWidget.Pivot.Left;
			}
			else if (offset.x == 1f)
			{
				if (offset.y == 0f) return NGUIWidget.Pivot.BottomRight;
				if (offset.y == 1f) return NGUIWidget.Pivot.TopRight;
				return NGUIWidget.Pivot.Right;
			}
			else
			{
				if (offset.y == 0f) return NGUIWidget.Pivot.Bottom;
				if (offset.y == 1f) return NGUIWidget.Pivot.Top;
		...
		static public void MoveWidget(NGUIRect w, float x, float y)
		{
			MoveRect(w, x, y);
		}
		...
		static public void MoveRect(NGUIRect rect, float x, float y)
		{
		...
		static public void ResizeWidget(NGUIWidget w, NGUIWidget.Pivot pivot, float x, float y, int minWidth, int minHeight)
		{
		...
		static public void ResizeWidget(NGUIWidget w, NGUIWidget.Pivot pivot, float x, float y, int minWidth,
		                                int      minHeight,
		                                int      maxWidth, int maxHeight)
		{
			if (pivot == NGUIWidget.Pivot.Center)
			{
		...
		static public void AdjustWidget(NGUIWidget w, float left, float bottom, float right, float top)
		{
		...
		static public void AdjustWidget(NGUIWidget w, float left, float bottom, float right, float top, int minWidth,
		                                int      minHeight)
		{
		                                ...
		                                int      minWidth, int   minHeight, int   maxWidth, int   maxHeight)
		{
			...
			{
				if (iBottom != 0)
					constraint.y -= Mathf.Lerp(limitHeight - finalHeight, 0f, piv.y);
				else
					constraint.y +=
						Mathf.Lerp(0f, limitHeight - finalHeight, piv.y);
