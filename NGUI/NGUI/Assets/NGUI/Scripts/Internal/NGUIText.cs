    ...
    static public class NGUIText
    {
                ...
                case Alignment.Right:
                    {
                        ...
                        for (int i = indexOffset, imax = verts.Size; i < imax; ++i)
                        {
                ...
                case Alignment.Justified:
                    {
                        ...
                        for (int i = indexOffset + elements, charIndex = 1, imax = verts.Size; i < imax; ++charIndex)
                        {
                            ...
                            if (elements == 4)
                            {
                                v = verts.Buffer[i]; v.x = x0; verts.Buffer[i++] = v;
                                v = verts.Buffer[i]; v.x = x0; verts.Buffer[i++] = v;
                                v = verts.Buffer[i]; v.x = x1; verts.Buffer[i++] = v;
                                v = verts.Buffer[i]; v.x = x1; verts.Buffer[i++] = v;
                            }
                            else if (elements == 2)
                            {
        ...
        static public int GetExactCharacterIndex(BetterList<Vector3> verts, BetterList<int> indices, Vector2 pos)
        {
            for (int i = 0, imax = indices.Size; i < imax; ++i)
            {
                ...
                float x0 = verts.Buffer[i0].x;
                if (pos.x < x0) continue;

                float x1 = verts.Buffer[i1].x;
                if (pos.x > x1) continue;

                float y0 = verts.Buffer[i0].y;
                if (pos.y < y0) continue;

                float y1 = verts.Buffer[i1].y;
                if (pos.y > y1) continue;

                return indices.Buffer[i];
            }
            return 0;
        }
        ...
        static public int GetApproximateCharacterIndex(BetterList<Vector3> verts, BetterList<int> indices, Vector2 pos)
        {
            ...
            {
                float diffY = Mathf.Abs(pos.y - verts.Buffer[i].y);
                if (diffY > bestY) continue;

                float diffX = Mathf.Abs(pos.x - verts.Buffer[i].x);
            ...
            return indices.Buffer[bestIndex];
        }
