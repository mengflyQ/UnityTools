namespace NGUI
{
    ...
    static public class NGUITools
    {
        ...
        static public Camera FindCameraForLayer(int layer)
        {
            ...
            for (int i = 0; i < NGUICamera.list.Size; ++i)
            {
                cam = NGUICamera.list.Buffer[i].cachedCamera;
        ...
        static public void AddWidgetCollider(GameObject go, bool considerInactive)
        {
            if (go != null)
            {
                ...
                NGUICamera ui = NGUICamera.FindCameraForLayer(go.layer);

                if (ui != null && (ui.eventType == NGUICamera.EventType.World_2D ||
                                   ui.eventType == NGUICamera.EventType.UI_2D))
                {
...
#endif
                    NGUIWidget widget = go.GetComponent<NGUIWidget>();
        ...
        static public void UpdateWidgetCollider(NGUIWidget w)
        {
            if (w == null) return;
        ...
        static public void UpdateWidgetCollider(NGUIWidget w, BoxCollider box)
        {
            if (box != null && w != null)
            {
                var dr = w.drawRegion;
        ...
        static public void UpdateWidgetCollider(NGUIWidget w, BoxCollider2D box)
        {
            if (box != null && w != null)
            {
                var dr = w.drawRegion;
        ...
        static public void CheckForPrefabStage(GameObject gameObject)
        {
            ...
            var rootsInParents = gameObject.GetComponentsInParent<NGUIRoot>(true);
            var panelsInParents = gameObject.GetComponentsInParent<NGUIPanel>(true);
            ...
            GameObject container =
                UnityEditor.EditorUtility.CreateGameObjectWithHideFlags("NGUIRoot (Environment)", HideFlags.DontSave);
            container.layer = instanceRoot.gameObject.layer;

            if (missingRoot)
                container.AddComponent<NGUIRoot>();

            if (missingPanel)
                container.AddComponent<NGUIPanel>();
		...
		Profiler.BeginSample("Editor-only GC allocation (GetComponent)");
            ...
            var widgets = go.GetComponentsInChildren<NGUIWidget>();
#if UNITY_5_5_OR_NEWER
            UnityEngine.Profiling.Profiler.EndSample();
#else
		Profiler.EndSample();
        ...
        static public int CalculateNextDepth(GameObject go)
        {
            ...
            {
                int depth = -1;
                NGUIWidget[] widgets = go.GetComponentsInChildren<NGUIWidget>();
                for (int i = 0, imax = widgets.Length; i < imax; ++i)
...
#else
                    if (w.cachedGameObject != go &&
                        (w.GetComponent<Collider>() != null || w.GetComponent<Collider2D>() != null)) continue;
#endif
        ...
        static public int AdjustDepth(GameObject go, int adjustment)
        {
            if (go != null)
            {
                NGUIPanel panel = go.GetComponent<NGUIPanel>();

                if (panel != null)
                {
                    NGUIPanel[] panels = go.GetComponentsInChildren<NGUIPanel>(true);

                    for (int i = 0; i < panels.Length; ++i)
                    {
                        NGUIPanel p = panels[i];
#if UNITY_EDITOR
                ...
                {
                    panel = FindInParents<NGUIPanel>(go);
                    if (panel == null) return 0;

                    NGUIWidget[] widgets = go.GetComponentsInChildren<NGUIWidget>(true);
        ...
        {
            NormalizeWidgetDepths(FindActive<NGUIWidget>());
        }
        ...
        {
            NormalizeWidgetDepths(go.GetComponentsInChildren<NGUIWidget>());
        }
        ...
        static public void NormalizeWidgetDepths(NGUIWidget[] list)
        {
            int size = list.Length;

            if (size > 0)
            {
                Array.Sort(list, NGUIWidget.FullCompareFunc);
        ...
        {
            NGUIPanel[] list = FindActive<NGUIPanel>();
            int size = list.Length;

            if (size > 0)
            {
                Array.Sort(list, NGUIPanel.CompareFunc);
        ...
        static public NGUIPanel CreateUI(bool advanced3D)
        {
        ...
        static public NGUIPanel CreateUI(bool advanced3D, int layer)
        {
        ...
        static public NGUIPanel CreateUI(Transform trans, bool advanced3D, int layer)
        {
            // Find the existing UI Root
            NGUIRoot root = (trans != null) ? NGUITools.FindInParents<NGUIRoot>(trans.gameObject) : null;

            if (root == null && NGUIRoot.list.Count > 0)
            {
                foreach (NGUIRoot r in NGUIRoot.list)
                {
            ...
            if (root == null)
            {
                for (int i = 0, imax = NGUIPanel.list.Count; i < imax; ++i)
                {
                    NGUIPanel p = NGUIPanel.list[i];
            ...
            if (root != null)
            {
                NGUICamera cam = root.GetComponentInChildren<NGUICamera>();
            ...
            if (root == null)
            {
                ...
                {
                    go.name = "UI Root (3D)";
                    root.scalingStyle = NGUIRoot.Scaling.Constrained;
                }
                else
                {
                    go.name = "UI Root";
                    root.scalingStyle = NGUIRoot.Scaling.Flexible;
                }
            ...
            NGUIPanel panel = root.GetComponentInChildren<NGUIPanel>();
            ...
            if (panel == null)
            {
                ...
                panel = root.gameObject.AddComponent<NGUIPanel>();
#if UNITY_EDITOR
        ...
        static public T AddWidget<T>(this GameObject go, int depth = int.MaxValue) where T : NGUIWidget
        {
        ...
        static public NGUISprite AddSprite(this GameObject go, INGUIAtlas atlas, string spriteName,
                                           int depth = int.MaxValue)
        {
            NGUISpriteData sp = (atlas != null) ? atlas.GetSprite(spriteName) : null;
            NGUISprite sprite = AddWidget<NGUISprite>(go, depth);
            sprite.type = (sp == null || !sp.hasBorder) ? NGUISprite.Type.Simple : NGUISprite.Type.Sliced;
            sprite.atlas = atlas;
        ...
        {
            var w = t.GetComponent<NGUIWidget>();
        ...
        static public void MakePixelPerfect(Transform t)
        {
            ...
            if (t.GetComponent<NGUIAnchor>() == null && t.GetComponent<NGUIRoot>() == null)
            {
#if UNITY_EDITOR
        ...
        static public void FitOnScreen(this Camera cam, Transform t, bool considerInactive = false,
        ...
        /// Example: uiCamera.FitOnScreen(contentObjectTransform, NGUICamera.lastEventPosition);
        /// </summary>
        ...
        /// <summary>
        /// Fit the specified NGUI hierarchy on the screen.
        /// Example: uiCamera.FitOnScreen(rootObjectTransform, contentObjectTransform, NGUICamera.lastEventPosition);
        /// </summary>
        ...
        static public void FitOnScreen(this Camera cam, Transform transform, Transform content, Vector3 pos,
                                       out Bounds bounds, bool considerInactive = false)
        {
        ...
        {
            NGUIRect[] rects = go.GetComponentsInChildren<NGUIRect>();
        ...
        {
            ExecuteAll<NGUIWidget>(root, "Start");
            ExecuteAll<NGUIPanel>(root, "Start");
            ExecuteAll<NGUIWidget>(root, "Update");
            ExecuteAll<NGUIPanel>(root, "Update");
            ExecuteAll<NGUIPanel>(root, "LateUpdate");
        }
#endif
                    ...
                    if (s_GetSizeOfMainGameView == null && !mCheckedMainViewFunc)
                    {
                        ...
                        if (methodInfo != null)
                        {
                            s_GetSizeOfMainGameView =
                                (Func<Vector2>)Delegate.CreateDelegate(typeof(Func<Vector2>), methodInfo);
                        }
	...
	static public Vector2 screenSize { get { return new Vector2(Screen.width, Screen.height); } }
        ...
        static Dictionary<string, NGUIWidget> mWidgets = new Dictionary<string, NGUIWidget>();
        static NGUIPanel mRoot;
        static GameObject mGo;

        public delegate void OnInitFunc<T>(T w) where T : NGUIWidget;
        ...
        static public T Draw<T>(string id, OnInitFunc<T> onInit = null) where T : NGUIWidget
        {
            NGUIWidget w;
            ...
            if (mRoot == null)
            {
                NGUICamera baseCam = null;
                NGUIRoot baseRoot = null;

                for (int i = 0; i < NGUIRoot.list.Count; ++i)
                {
                    NGUIRoot root = NGUIRoot.list[i];

                    if (root)
                    {
                        NGUICamera cam = NGUICamera.FindCameraForLayer(root.gameObject.layer);
                ...
                {
                    mRoot = baseRoot.gameObject.AddChild<NGUIPanel>();
                }
        ...
        {
            var list = FindActive<NGUISprite>();
            ...
            var labels = FindActive<NGUILabel>();
