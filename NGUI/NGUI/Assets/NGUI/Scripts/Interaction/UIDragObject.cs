namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/Event/Drag Object")]
	public class UIDragObject : MonoBehaviour
	{
		...
		public NGUIRect contentRect = null;
		...
		void OnEnable()
		{
			...
			{
				NGUIWidget w                 = target.GetComponent<NGUIWidget>();
		...
		void FindPanel()
		{
			panelRegion = (target != null) ? NGUIPanel.Find(target.transform.parent) : null;
		...
		{
			if (NGUICamera.currentTouchID == -2 || NGUICamera.currentTouchID == -3) return;
			...
			if (enabled && NGUITools.GetActive(gameObject) && target != null)
			{
				if (pressed)
				{
					if (!mPressed)
					{
						...
						mTouchID = NGUICamera.currentTouchID;
						mPressed = true;
						mStarted = false;
						CancelMovement();
						...
						Transform trans = NGUICamera.currentCamera.transform;
						mPlane =
							new
								Plane((panelRegion != null ? panelRegion.cachedTransform.rotation : trans.rotation) * Vector3.back,
								      NGUICamera.lastWorldPosition);
					}
				}
				else if (mPressed && mTouchID == NGUICamera.currentTouchID)
				{
					mPressed = false;
		...
		{
			if (mPressed && mTouchID == NGUICamera.currentTouchID && enabled && NGUITools.GetActive(gameObject) &&
			    target               != null)
			{
				NGUICamera.currentTouch.clickNotification = NGUICamera.ClickNotification.BasedOnDelta;

				Ray   ray  = NGUICamera.currentCamera.ScreenPointToRay(NGUICamera.currentTouch.pos);
				float dist = 0f;
				...
				if (mPlane.Raycast(ray, out dist))
				{
					...
					if (restrictWithinPanel)
					{
						...
						if (dragEffect != DragEffect.MomentumAndSpring &&
						    panelRegion.ConstrainTargetToBounds(target, ref mBounds, true))
							CancelMovement();
					}
				}
			}
