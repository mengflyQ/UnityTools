namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/Effect/Button Color")]
	public class UIButtonColor : UIWidgetContainer
	{
		...
		[System.NonSerialized] protected NGUIWidget mWidget;
		...
		protected virtual void OnInit()
		{
			...
			if (tweenTarget != null) mWidget                               = tweenTarget.GetComponent<NGUIWidget>();
...
#endif
			if (mInitDone) OnHover(NGUICamera.IsHighlighted(gameObject));

			if (NGUICamera.currentTouch != null)
			{
				if (NGUICamera.currentTouch.pressed      == gameObject) OnPress(true);
				else if (NGUICamera.currentTouch.current == gameObject) OnHover(true);
			}
		}
		...
		protected virtual void OnPress(bool isPressed)
		{
			if (isEnabled)
			{
				...
				if (tweenTarget != null)
				{
					...
					else if (NGUICamera.currentTouch != null && NGUICamera.currentTouch.current == gameObject)
					{
						if (NGUICamera.currentScheme == NGUICamera.ControlScheme.Controller)
						{
							SetState(State.Hover, false);
						}
						else if (NGUICamera.currentScheme == NGUICamera.ControlScheme.Mouse &&
