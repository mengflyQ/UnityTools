...
namespace NGUI
{
    ...
    public class UIWrapContentEx : MonoBehaviour
    {
        ...
        protected virtual void Start()
        {
            ...
            WrapContent();
            if (mScroll != null) mScroll.GetComponent<NGUIPanel>().onClipMove = OnMove;
            mFirstTime = false;
        ...
        /// </summary>
        protected virtual void OnMove(NGUIPanel panel)
        {
            WrapContent();
        }
        ...
        public virtual void ResetChildPositions()
        {
            ...
            for (int i = 0, imax = MChildren.Count; i < imax; ++i)
            {
                ...
                t.localPosition = mHorizontal
                                      ? new Vector3(i * itemSize, pos.y,         0f)
                                      : new Vector3(pos.x,        -i * itemSize, 0f);
        ...
        public virtual void Reposition()
        {
            for (int i = 0, imax = MChildren.Count; i < imax; ++i)
            {
                Transform t     = MChildren[i];
                Vector3   pos   = t.localPosition;
                int       index = GetRealIndex(t.localPosition);
                if (mHorizontal)
                {
                    t.localPosition = new Vector3(index * itemSize, pos.y, 0f);
                }
                else
                {
                    t.localPosition = new Vector3(pos.x, -index * itemSize, 0f);
                }
            }
        }
        ...
        public virtual void WrapContent()
        {
            ...
            if (mHorizontal)
            {
                ...
                for (int i = 0, imax = MChildren.Count; i < imax; ++i)
                {
                    ...
                    if (wrapInCircle)
                        index = (index % (maxIndex - minIndex) + (maxIndex - minIndex)) % (maxIndex - minIndex) +
                                minIndex;
                    if (distance < -extents)
                    {
                        Vector3 pos = t.localPosition;
                        ...
                        if (wrapInCircle)
                            realIndex = (realIndex % (maxIndex - minIndex) + (maxIndex - minIndex)) %
                    ...
                    if (cullContent)
                    {
                        ...
                        if (!NGUICamera.IsPressed(t.gameObject))
        ...
        public int GetRealIndex(Vector3 localPos)
        {
            ...
            if (wrapInCircle)
                realIndex = (realIndex % (maxIndex - minIndex) + (maxIndex - minIndex)) % (maxIndex - minIndex) +
                            minIndex;
            return realIndex;
        }
