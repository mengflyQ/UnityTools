namespace NGUI
{
	...
	[AddComponentMenu("NGUI/Event/Button Scale")]
	public class UIButtonScale : MonoBehaviour
	{
		...
		void OnPress(bool isPressed)
		{
			...
			{
				if (!mStarted) Start();
				TweenScale.Begin(tweenTarget.gameObject, duration,
				                 isPressed
					                 ? Vector3.Scale(mScale, pressed)
					                 : (NGUICamera.IsHighlighted(gameObject) ? Vector3.Scale(mScale, hover) : mScale))
		...
		void OnHover(bool isOver)
		{
			...
			{
				if (!mStarted) Start();
				TweenScale.Begin(tweenTarget.gameObject, duration, isOver ? Vector3.Scale(mScale, hover) : mScale)
				          .method = UITweener.Method.EaseInOut;
			}
		}
