namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/Event/Popup List")]
	public class UIPopupList : UIWidgetContainer
	{
		...
		public NGUILabel.Modifier textModifier = NGUILabel.Modifier.None;
		...
		[HideInInspector] [SerializeField] protected NGUIPanel       mPanel;
		[HideInInspector] [SerializeField] protected NGUIBasicSprite mBackground;
		[HideInInspector] [SerializeField] protected NGUIBasicSprite mHighlight;
		[HideInInspector] [SerializeField] protected NGUILabel       mHighlightedLabel = null;
		[HideInInspector] [SerializeField] protected List<NGUILabel> mLabelList        = new List<NGUILabel>();
		...
		[HideInInspector] [SerializeField] NGUILabel textLabel;
		...
		{
			get { return current != null && (mChild != null || mFadeOutComplete > Time.unscaledTime); }
		}
		...
		protected virtual void Highlight(NGUILabel lbl, bool instant)
		{
			if (mHighlight != null)
			{
		...
		{
			Select(go.GetComponent<NGUILabel>(), true);
		...
		protected virtual void OnKey(KeyCode key)
		{
			if (enabled && current == this)
			{
				if (key == NGUICamera.current.cancelKey0 || key == NGUICamera.current.cancelKey1)
					OnSelect(false);
			}
		}
		...
		protected virtual void OnSelect(bool isSelected)
		{
			if (!isSelected)
			{
				var sel = NGUICamera.selectedObject;
				if (sel == null || !(sel == mChild ||
				                     (mChild != null && sel != null &&
				                      NGUITools.IsChild(mChild.transform, sel.transform))))
					CloseSelf();
			}
		}
		...
		public virtual void CloseSelf()
		{
			if (mChild != null && current == this)
			{
				...
				if (isAnimated)
				{
					...
					Collider[] cols =
						mChild.GetComponentsInChildren<Collider>();
		...
		protected virtual void AnimateColor(NGUIWidget widget)
		{
		...
		protected virtual void AnimatePosition(NGUIWidget widget, bool placeAbove, float bottom)
		{
		...
		protected virtual void AnimateScale(NGUIWidget widget, bool placeAbove, float bottom)
		{
			GameObject go = widget.gameObject;
			...
			var minHeight =
				activeFontSize * activeFontScale +
				mBgBorder      * 2f;
			t.localScale =
				new Vector3(fitScale, fitScale * minHeight / widget.height, fitScale);
			...
			if (placeAbove)
			{
				...
				t.localPosition =
					new Vector3(pos.x, pos.y - fitScale * widget.height + fitScale * minHeight, pos.z);
		...
		protected void Animate(NGUIWidget widget, bool placeAbove, float bottom)
		{
			AnimateColor(widget);
		...
		protected virtual void OnClick()
		{
			...
			if (mChild == null)
			{
				...
				if (openOn == OpenOn.RightClick && NGUICamera.currentTouchID != -2) return;
				Show();
			}
		...
		IEnumerator CloseIfUnselected()
		{
			for (;;)
			{
				...
				if (sel != mSelection &&
				    (sel == null || !(sel == mChild || NGUITools.IsChild(mChild.transform, sel.transform))))
				{
					CloseSelf();
					break;
				}
		...
		public virtual void Show()
		{
			if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
			{
				...
				NGUICamera.selectedObject = (NGUICamera.hoveredObject ?? gameObject);
				mSelection              = NGUICamera.selectedObject;
				source                  = mSelection;
				...
				if (mPanel == null)
				{
					mPanel = NGUIPanel.Find(transform);
				...
				if (separatePanel)
				{
					...
					var panel = mChild.AddComponent<NGUIPanel>();
					panel.depth        = 1000000;
				...
				if (separatePanel)
				{
					var root                                         = mPanel.GetComponentInParent<NGUIRoot>();
					if (root == null && NGUIRoot.list.Count != 0) root = NGUIRoot.list[0];
				...
				{
					startingPosition = NGUICamera.lastEventPosition;
					min =
						pTrans.InverseTransformPoint(mPanel.anchorCamera.ScreenToWorldPoint(startingPosition));
					max              = min;
					t.localPosition  = min;
				...
				{
					NGUI2DSprite sp2 = mChild.AddWidget<NGUI2DSprite>(depth);
				...
				{
					NGUICamera cam = NGUICamera.FindCameraForLayer(mSelection.layer);
				...
				mBackground.cachedTransform.localPosition =
					new Vector3(0f, placeAbove ? bgPadding.y * 2f - overlap : overlap, 0f);
				...
				float x         = 0f,
				              y = placeAbove ? bgPadding.y - padding.y - overlap : -padding.y - bgPadding.y + overlap;
				...
				for (int i = 0, imax = items.Count; i < imax; ++i)
				{
					...
					NGUILabel lbl = NGUITools.AddWidget<NGUILabel>(mChild, mBackground.depth + 2);
					lbl.name         = i.ToString();
					...
					lbl.color        = textColor;
					lbl.cachedTransform.localPosition =
						new Vector3(bgPadding.x + padding.x - lbl.pivotOffset.x, y, -1f);
					lbl.overflowMethod = NGUILabel.Overflow.ResizeFreely;
				...
				{
					NGUILabel lbl = labels[i];
					lbl.overflowMethod = NGUILabel.Overflow.ShrinkContent;
					lbl.width          = lblWidth;
				}
				...
				var absoluteParent = mPanel; // NGUIRoot.list[0].GetComponent<NGUIPanel>();

				for (;;)
				{
					var p = absoluteParent.parent;
					if (p == null) break;
					var pp = p.GetComponentInParent<NGUIPanel>();
					if (pp == null) break;
					absoluteParent = pp;
				}
				...
				if (pTrans != null)
				{
					...
					var adj = NGUIRoot.GetPixelSizeAdjustment(gameObject);
					min /= adj;
					max /= adj;
				}
