namespace NGUI
{
    ...
    [AddComponentMenu("NGUI/Event/Key Navigation")]
    public class UIKeyNavigation : MonoBehaviour
    {
        ...
        static public UIKeyNavigation current
        {
            get
            {
                GameObject go = NGUICamera.hoveredObject;
        ...
        public GameObject Get(Vector3 myDir, float x = 1f, float y = 1f)
        {
            ...
            for (int i = 0; i < list.Size; ++i)
            {
                ...
                var widget = nav.GetComponent<NGUIWidget>();
        ...
        static protected Vector3 GetCenter(GameObject go)
        {
            ...
            var cam = NGUICamera.FindCameraForLayer(go.layer);
        ...
        public virtual void OnNavigate(KeyCode key)
        {
            ...
            if (go != null) NGUICamera.hoveredObject = go;
        }
        ...
        public virtual void OnKey(KeyCode key)
        {
            ...
            if (key == KeyCode.Tab)
            {
                ...
                if (go == null)
                {
                    if (NGUICamera.GetKey(KeyCode.LeftShift) || NGUICamera.GetKey(KeyCode.RightShift))
                    {
                        go = GetLeft();
                        if (go == null) go = GetUp();
                ...
                if (go != null)
                {
                    NGUICamera.currentScheme = NGUICamera.ControlScheme.Controller;
                    NGUICamera.hoveredObject = go;
                    NGUIInputField inp = go.GetComponent<NGUIInputField>();
        ...
        protected virtual void OnClick()
        {
            if (NGUITools.GetActive(onClick))
                NGUICamera.hoveredObject = onClick;
        }
    }
}