namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/Event/Play Animation")]
	public class UIPlayAnimation : MonoBehaviour
	{
		...
		void OnEnable()
		{
			...
			if (NGUICamera.currentTouch != null)
			{
				if (trigger == Trigger.OnPress || trigger == Trigger.OnPressTrue)
					mActivated = (NGUICamera.currentTouch.pressed == gameObject);

				if (trigger == Trigger.OnHover || trigger == Trigger.OnHoverTrue)
					mActivated = (NGUICamera.currentTouch.current == gameObject);
			}
		...
		void OnDragOver()
		{
			...
			{
				if (NGUICamera.currentTouch.dragged == gameObject) Play(true,      true);
		...
		void OnDragOut()
		{
			if (enabled && dualState && NGUICamera.hoveredObject != gameObject)
				Play(false, true);
		}

		void OnDrop(GameObject go)
		{
			if (enabled && trigger == Trigger.OnPress && NGUICamera.currentTouch.dragged != gameObject)
				Play(false, true);
		}
		...
		public void Play(bool forward, bool onlyIfDifferent)
		{
			if (target || animator)
			{
				...
				if (clearSelection && NGUICamera.selectedObject == gameObject)
					NGUICamera.selectedObject = null;

				int       pd  = -(int) playDirection;
				Direction dir = forward ? playDirection : ((Direction) pd);
				NGUIActiveAnimation anim = target
					                           ? NGUIActiveAnimation.Play(target, clipName, dir, ifDisabledOnPlay,
					                                                      disableWhenFinished)
					                           : NGUIActiveAnimation.Play(animator, clipName, dir, ifDisabledOnPlay,
					                                                      disableWhenFinished);
