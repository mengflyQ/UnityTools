namespace NGUI
{
	...
	/// 想过在可拖动面板中自动居中一个对象吗?将这个脚本附加到容器上，容器的子对象以其为中心。
	/// </summary>

	[AddComponentMenu("NGUI/Event/Center Scroll View on Child")]
	public class UICenterOnChild : MonoBehaviour
	{
		...
		public NGUISpringPanel.OnFinished onFinished;
		...
		public void Recenter()
		{
			if (mScrollView == null)
			{
				...
				if (mScrollView == null)
				{
					Debug.LogWarning(GetType() + " requires " + typeof(UIScrollView) + " on a parent object in order to work",
					                 this);
					enabled = false;
					return;
				}
			...
			if (nextPageThreshold > 0f && NGUICamera.currentTouch != null)
			{
				// If we're still on the same object
				if (mCenteredObject           != null &&
				    mCenteredObject.transform == (list != null ? list[index] : trans.GetChild(index)))
				{
					Vector3 totalDelta = NGUICamera.currentTouch.totalDelta;
					...
					if (Mathf.Abs(delta) > nextPageThreshold)
					{
						if (delta > nextPageThreshold)
						{
							...
							else if (ignoredIndex > 0)
							{
								...
								closest = (GetComponent<UIWrapContent>() == null)
									          ? trans.GetChild(0)
									          : trans.GetChild(trans.childCount - 1);
						}
		...
		void CenterOn(Transform target, Vector3 panelCenter)
		{
			if (target != null && mScrollView != null && mScrollView.panel != null)
			{
...
#endif
				{
					...
					pos.z =
						Mathf.Round(pos.z);
					NGUISpringPanel.Begin(mScrollView.panel.cachedGameObject, pos, springStrength).onFinished = onFinished;
				}
			}
