namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/Event/NGUI Slider")]
	public class UISlider : UIProgressBar
	{
		...
		protected override void Upgrade()
		{
			if (direction != Direction.Upgraded)
			{
				...
				if (foreground != null)
					mFG = foreground.GetComponent<NGUIWidget>();
		...
		GameObject bg =
 (mBG != null && (mBG.collider != null || mBG.GetComponent<Collider2D>() != null)) ? mBG.gameObject : gameObject;
		NGUIEventListener bgl = NGUIEventListener.Get(bg);
		...
		if (thumb != null && (thumb.collider != null || thumb.GetComponent<Collider2D>() != null) && (mFG == null || thumb != mFG.cachedTransform))
#else
			GameObject bg =
				(mBG != null && (mBG.GetComponent<Collider>() != null || mBG.GetComponent<Collider2D>() != null))
					? mBG.gameObject
					: gameObject;
			NGUIEventListener bgl = NGUIEventListener.Get(bg);
			bgl.onPress += OnPressBackground;
			...
			if (thumb != null && (thumb.GetComponent<Collider>() != null || thumb.GetComponent<Collider2D>() != null) &&
			    (mFG   == null ||
			     thumb != mFG.cachedTransform))
#endif
			{
				NGUIEventListener fgl = NGUIEventListener.Get(thumb.gameObject);
		...
		{
			if (NGUICamera.currentScheme == NGUICamera.ControlScheme.Controller) return;
			mCam  = NGUICamera.currentCamera;
			value = ScreenToValue(NGUICamera.lastEventPosition);
		...
		protected void OnPressForeground(GameObject go, bool isPressed)
		{
			...
			if (isPressed)
			{
				mOffset = (mFG == null) ? 0f : value - ScreenToValue(NGUICamera.lastEventPosition);
			}
		...
		protected void OnDragForeground(GameObject go, Vector2 delta)
		{
			if (NGUICamera.currentScheme == NGUICamera.ControlScheme.Controller) return;
			mCam  = NGUICamera.currentCamera;
			value = mOffset + ScreenToValue(NGUICamera.lastEventPosition);
		}
