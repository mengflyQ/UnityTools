namespace NGUI
{
	...
	[AddComponentMenu("NGUI/Event/Scroll View")]
	public class UIScrollView : MonoBehaviour
	{
		...
		public NGUIWidget.Pivot contentPivot = NGUIWidget.Pivot.TopLeft;
		...
		{
			get { return mPressed && mDragStarted; }
		}
		...
		void Awake()
		{
			...
			if (mPanel.clipping == NGUIDrawCall.Clipping.None)
				mPanel.clipping = NGUIDrawCall.Clipping.ConstrainButDontClip;
			...
			// Auto-upgrade
			if (contentPivot == NGUIWidget.Pivot.TopLeft && relativePositionOnReset != Vector2.zero)
			{
				contentPivot =
					NGUIMath.GetPivot(new Vector2(relativePositionOnReset.x, 1f - relativePositionOnReset.y));
		...
		void CheckScrollbars()
		{
			if (horizontalScrollBar != null)
			{
				...
				horizontalScrollBar.alpha =
					((showScrollBars == ShowCondition.Always) || shouldMoveHorizontally) ? 1f : 0f;
			}
		...
		public bool RestrictWithinBounds(bool instant, bool horizontal, bool vertical)
		{
			...
			if (constraint.sqrMagnitude > 0.1f)
			{
				if (!instant && dragEffect == DragEffect.MomentumAndSpring)
				{
					...
					NGUISpringPanel.Begin(mPanel.gameObject, pos, 8f);
				}
				else
				{
		...
		{
			NGUISpringPanel sp             = GetComponent<NGUISpringPanel>();
		...
		protected void UpdateScrollbars(UIProgressBar slider,   float contentMin, float contentMax, float contentSize,
		                                float         viewSize, bool  inverted)
		{
			...
			mIgnoreCallbacks = true;
			{
				...
				if (viewSize < contentSize)
				{
					...
					slider.value = inverted
						               ? ((contentPadding > 0.001f) ? 1f - contentMin / contentPadding : 0f)
		...
		{
			if (mPressed == pressed || NGUICamera.currentScheme == NGUICamera.ControlScheme.Controller) return;
			...
			{
				if (!pressed && mDragID == NGUICamera.currentTouchID) mDragID = -10;
				...
				if (pressed)
				{
					...
					mLastPos = NGUICamera.lastWorldPosition;
				...
				{
					if (mDragStarted && restrictWithinPanel && mPanel.clipping != NGUIDrawCall.Clipping.None)
		...
		public void Drag()
		{
			if (!mPressed || NGUICamera.currentScheme == NGUICamera.ControlScheme.Controller) return;

			if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
			{
				if (mDragID == -10) mDragID = NGUICamera.currentTouchID;
				...
				if (mPlane.Raycast(ray, out dist))
				{
					...
					if (dragEffect == DragEffect.None) mMomentum = Vector3.zero;
						...
						mMomentum =
							Vector3.Lerp(mMomentum, mMomentum + offset * (0.01f * momentumAmount), 0.67f);
		...
		[HideInInspector] public UICenterOnChild centerOnChild = null;
		...
		void LateUpdate()
		{
			...
			if (!mPressed)
			{
				if (mMomentum.magnitude > 0.0001f || Mathf.Abs(mScroll) > 0.0001f)
				{
					...
					if (restrictWithinPanel && mPanel.clipping != NGUIDrawCall.Clipping.None)
					{
