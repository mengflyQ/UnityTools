namespace NGUI
{
	...
	[AddComponentMenu("NGUI/Event/Drag and Drop Item")]
	public class UIDragDropItem : MonoBehaviour
	{
		...
		[Tooltip("How long the user has to press on an item before the drag action activates.")] [HideInInspector]
		...
		[System.NonSerialized] protected NGUIRoot                mRoot;
		...
		[System.NonSerialized] protected NGUICamera.MouseOrTouch mTouch;
		...
		{
			foreach (var drag in draggedItems)
				if (drag.gameObject == go)
					return true;
			return false;
		}
		...
		protected virtual void OnDisable()
		{
			...
			{
				StopDragging(null);
				NGUICamera.onPress     -= OnGlobalPress;
				NGUICamera.onClick     -= OnGlobalClick;
				NGUICamera.onMouseMove -= OnDrag;
			}
		}
		...
		{
			if (!interactable || NGUICamera.currentTouchID == -2 || NGUICamera.currentTouchID == -3) return;

			if (isPressed)
			{
				if (!mPressed)
				{
					mTouch         = NGUICamera.currentTouch;
					mDragStartTime = RealTime.time + pressAndHoldDelay;
					mPressed       = true;
				}
			}
			else if (mPressed && mTouch == NGUICamera.currentTouch)
			{
				mPressed = false;
		...
		protected virtual void OnClick()
		{
			...
			if (clickToDrag && !mDragging && NGUICamera.currentTouchID == -1 && draggedItems.Count == 0)
			{
				mTouch = NGUICamera.currentTouch;
				...
				{
					NGUICamera.onMouseMove += item.OnDrag;
					NGUICamera.onPress     += item.OnGlobalPress;
					NGUICamera.onClick     += item.OnGlobalClick;
				}
			}
		}
		...
		{
			if (state && NGUICamera.currentTouchID != -1)
			{
		...
		protected void OnGlobalClick(GameObject go)
		{
			...
			if (NGUICamera.currentTouchID == -1) StopDragging(go);
		...
		protected virtual void OnDragStart()
		{
			...
			if (!enabled || mTouch != NGUICamera.currentTouch) return;
			...
			if (!mDragging)
			{
				if (cloneOnDrag)
				{
					...
					if (NGUICamera.currentTouch == null)
						NGUICamera.currentTouch = mTouch;

					mTouch = null;

					NGUICamera.Notify(gameObject, "OnPress", false);
					NGUICamera.Notify(gameObject, "OnHover", false);
					return item;
				}
				else
				{
		...
		{
			if (!interactable) return;
			if (!mDragging || !enabled || mTouch != NGUICamera.currentTouch) return;
		...
		protected virtual void OnDragEnd()
		{
			if (!interactable) return;
			if (!enabled || mTouch != NGUICamera.currentTouch) return;
			StopDragging(NGUICamera.lastHit.collider != null ? NGUICamera.lastHit.collider.gameObject : null);
		}
		...
		protected virtual void OnDragDropStart()
		{
			...
			mRoot   = NGUITools.FindInParents<NGUIRoot>(mParent);
