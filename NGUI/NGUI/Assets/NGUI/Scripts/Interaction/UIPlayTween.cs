namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/Event/Play Tween")]
		...
		void OnHover(bool isOver)
		{
			if (enabled)
			{
				    ...
				    (trigger == Trigger.OnHoverFalse && !isOver))
				{
					...
					if (!isOver && NGUICamera.hoveredObject != null &&
					    NGUICamera.hoveredObject.transform.IsChildOf(transform))
					{
						// Subscribe to a global hover listener so we can keep receiving hover notifications
						NGUICamera.onHover += CustomHoverListener;
						isOver           =  true;
