namespace NGUI
{
	...
	[AddComponentMenu("NGUI/Event/Button Offset")]
	public class UIButtonOffset : MonoBehaviour
	{
		...
		void OnPress(bool isPressed)
		{
			...
			if (enabled)
			{
				...
				TweenPosition.Begin(tweenTarget.gameObject, duration,
				                    isPressed
					                    ? mPos + pressed
					                    : (NGUICamera.IsHighlighted(gameObject) ? mPos + hover : mPos)).method =
		...
		void OnHover(bool isOver)
		{
			...
			{
				if (!mStarted) Start();
				TweenPosition.Begin(tweenTarget.gameObject, duration, isOver ? mPos + hover : mPos).method =
					UITweener.Method.EaseInOut;
			}
		}
