namespace NGUI
{
	...
	[ExecuteInEditMode]
	[AddComponentMenu("NGUI/Event/NGUI Progress Bar")]
	public class UIProgressBar : UIWidgetContainer
	{
		...
		[HideInInspector] [SerializeField] protected NGUIWidget      mBG;
		[HideInInspector] [SerializeField] protected NGUIWidget      mFG;
		...
		public NGUIWidget foregroundWidget
		{
			get { return mFG; }
			set
			{
		...
		public NGUIWidget backgroundWidget
		{
			get { return mBG; }
			set
			{
		...
		public float alpha
		{
			...
			if (mFG != null)
			{
				...
				else if (mFG.GetComponent<Collider2D>() != null) mFG.GetComponent<Collider2D>().enabled =
 mFG.alpha > 0.001f;
			}
			...
			if (mBG != null)
			{
				...
				else if (mBG.GetComponent<Collider2D>() != null) mBG.GetComponent<Collider2D>().enabled =
 mBG.alpha > 0.001f;
			}

			if (thumb != null)
			{
				NGUIWidget w = thumb.GetComponent<NGUIWidget>();
				
				if (w != null)
				{
					w.alpha = value;
					if (w.collider != null) w.collider.enabled = w.alpha > 0.001f;
					else if (w.GetComponent<Collider2D>() != null) w.GetComponent<Collider2D>().enabled =
 w.alpha > 0.001f;
				}
			}
#else
				...
				if (thumb != null)
				{
					...
					if (w != null)
					{
						...
						if (w.GetComponent<Collider>() != null)
							w.GetComponent<Collider>().enabled = w.alpha > 0.001f;
						else if (w.GetComponent<Collider2D>() != null)
							w.GetComponent<Collider2D>().enabled = w.alpha > 0.001f;
					}
				}
#endif
			}
		...
		public virtual void ForceUpdate()
		{
			...
			if (mFG != null)
			{
				NGUIBasicSprite sprite = mFG as NGUIBasicSprite;

				if (isHorizontal)
				{
					if (sprite != null && sprite.type == NGUIBasicSprite.Type.Filled)
					{
						if (sprite.fillDirection == NGUIBasicSprite.FillDirection.Horizontal ||
						    sprite.fillDirection == NGUIBasicSprite.FillDirection.Vertical)
						{
							sprite.fillDirection = NGUIBasicSprite.FillDirection.Horizontal;
					...
					{
						mFG.drawRegion =
							isInverted ? new Vector4(1f - value, 0f, 1f, 1f) : new Vector4(0f, 0f, value, 1f);
						mFG.enabled = true;
						turnOff     = value < 0.001f;
					}
				}
				else if (sprite != null && sprite.type == NGUIBasicSprite.Type.Filled)
				{
					if (sprite.fillDirection == NGUIBasicSprite.FillDirection.Horizontal ||
					    sprite.fillDirection == NGUIBasicSprite.FillDirection.Vertical)
					{
