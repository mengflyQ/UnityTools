namespace NGUIEditor
{
	...
	static public class FreeType
	{
		...
		static public bool CreateFont(Font       ttf,        int           size, int faceIndex, bool kerning,
		                              string     characters, int           padding,
		                              out BMFont font,       out Texture2D tex)
		{
			font = null;
			tex  = null;
			...
			else
			{
				...
				FT_GlyphSlotRec space =
					(FT_GlyphSlotRec) Marshal.PtrToStructure(faceRec.glyph, typeof(FT_GlyphSlotRec));
				...
				foreach (char ch in characters)
				{
					...
					if (glyph.bitmap.width > 0 && glyph.bitmap.rows > 0)
					{
						...
						Texture2D texture = new Texture2D(glyph.bitmap.width, glyph.bitmap.rows,
						                                  UnityEngine.TextureFormat.ARGB32, false);
