...
namespace NGUIEditor
{
	...
	public class UIPopupListInspector : UIWidgetContainerEditor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (GUI.changed)
			{
				serializedObject.FindProperty("mSelectedItem").stringValue =
					(sp.boolValue && mList.items.Count > 0) ? mList.items[0] : "";
			}
			...
			EditorGUI.BeginDisabledGroup(!sp.boolValue);
			{
				GUI.changed = false;
				string sel =
					NGUIEditorTools.DrawList("Initial Value", mList.items.ToArray(), mList.value);
		...
		void DrawAtlas()
		{
			if (NGUIEditorTools.DrawHeader("Atlas"))
			{
				...
				{
					NGUIEditorTools.DrawPaddedSpriteField("Background", mList.atlas as INGUIAtlas,
					                                      mList.backgroundSprite, OnBackground);
					NGUIEditorTools.DrawPaddedSpriteField("Highlight", mList.atlas as INGUIAtlas, mList.highlightSprite,
					                                      OnHighlight);
				}
				else
				{
		...
		void DrawFont()
		{
			if (NGUIEditorTools.DrawHeader("Font"))
			{
				...
				mType = FontType.Bitmap;
					...
					if (mType == FontType.NGUI)
					{
						var fnt = NGUIEditorTools.DrawProperty("", serializedObject, "bitmapFont",
						...
						if (GUI.changed)
						{
							...
							NGUISettings.ambigiousFont =
								fnt.objectReferenceValue;
						}
					}
					else
					{
						ttf = NGUIEditorTools.DrawProperty("", serializedObject, "trueTypeFont",
						                                   GUILayout.MinWidth(40f));
				...
				if (ttf != null && ttf.objectReferenceValue != null)
				{
					GUILayout.BeginHorizontal();
					{
						...
						NGUIEditorTools.DrawProperty("", serializedObject, "fontStyle",
						                             GUILayout.MinWidth(40f));
