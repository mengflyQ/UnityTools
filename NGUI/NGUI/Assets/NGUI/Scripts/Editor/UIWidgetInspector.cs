namespace NGUIEditor
{
	...
	[CustomEditor(typeof(NGUIWidget), true)]
	public class UIWidgetInspector : UIRectEditor
	{
		...
		static public NGUIWidget.Pivot[] pivotPoints =
		{
			NGUIWidget.Pivot.BottomLeft,
			NGUIWidget.Pivot.TopLeft,
			NGUIWidget.Pivot.TopRight,
			NGUIWidget.Pivot.BottomRight,
			NGUIWidget.Pivot.Left,
			NGUIWidget.Pivot.Top,
			NGUIWidget.Pivot.Right,
			NGUIWidget.Pivot.Bottom,
		};
		...
		NGUIWidget.Pivot mDragPivot   = NGUIWidget.Pivot.Center;
		...
		static public NGUIWidget.Pivot GetPivotUnderMouse(Vector3[]  worldPos, Event e, bool[] resizable, bool movable,
		                                                ref Action action)
		{
		...
		static public void DrawAnchorHandle(NGUIRect.AnchorPoint anchor, Transform myTrans, Vector3[] myCorners, int side,
		                                    int                id)
		{
		...
		public void OnSceneGUI()
		{
			...
			if (!NGUIWidget.showHandles) return;
			...
			bool   canResize = (mWidget.GetComponent<NGUIStretch>() == null);
			...
			NGUILabel lbl = mWidget as NGUILabel;

			if (lbl != null)
			{
				if (lbl.overflowMethod == NGUILabel.Overflow.ResizeFreely)
				{
				...
				else if (lbl.overflowMethod == NGUILabel.Overflow.ResizeHeight)
				{
					resizable[5] = false; // top
					resizable[7] = false; // bottom
				}
			...
			if (mWidget.keepAspectRatio == NGUIWidget.AspectRatioSource.BasedOnHeight)
			{
				resizable[4] = false;
				resizable[6] = false;
			}
			else if (mWidget.keepAspectRatio == NGUIWidget.AspectRatioSource.BasedOnWidth)
			{
				resizable[5] = false;
				resizable[7] = false;
			}
			...
			switch (type)
			{
							...
							if (Raycast(handles, out pos))
							{
								...
								if (mAction != Action.None)
								{
									...
									if (mAction == Action.Move)
									{
										...
										{
											// Snap the widget
											after = NGUISnap.Snap(after, mWidget.localCorners,
				...
				case EventType.MouseUp:
				{
					...
					{
						List<NGUIWidget> widgets =
							NGUIEditorTools.SceneViewRaycast(e.mousePosition);
		...
		static public void DrawColor(SerializedObject so, NGUIWidget w)
		{
			if ((w.GetType() != typeof(NGUIWidget)))
			{
		...
		static public void DrawInspectorProperties(SerializedObject so, NGUIWidget w, bool drawColor)
		{
			if (drawColor)
			{
				DrawColor(so, w);
		...
		static void DrawDimensions(SerializedObject so, NGUIWidget w, bool isPrefab)
		{
			GUILayout.BeginHorizontal();
			{
				...
				if (!freezeSize && lbl) freezeSize = (lbl.overflowMethod == NGUILabel.Overflow.ResizeFreely);
				...
				if (!freezeSize && lbl)
				{
					NGUILabel.Overflow ov = lbl.overflowMethod;
					freezeSize = (ov == NGUILabel.Overflow.ResizeFreely || ov == NGUILabel.Overflow.ResizeHeight);
				}
				...
				else
				{
					...
					if (GUILayout.Button("Snap", GUILayout.Width(60f)))
					{
						...
						{
							NGUIWidget pw = go.GetComponent<NGUIWidget>();
		...
		static void DrawDepth(SerializedObject so, NGUIWidget w, bool isPrefab)
		{
			GUILayout.Space(2f);
			GUILayout.BeginHorizontal();
			{
			...
			if (matchingDepths > 1)
			{
				EditorGUILayout.HelpBox(matchingDepths + " widgets are sharing the depth value of " + w.depth,
				                        MessageType.Info);
			}
		}
		...
		static void DrawPivot(SerializedObject so, NGUIWidget w)
		{
			...
			else
			{
				...
				Toggle(w, "\u25C4", "ButtonLeft",  NGUIWidget.Pivot.Left,   true);
				Toggle(w, "\u25AC", "ButtonMid",   NGUIWidget.Pivot.Center, true);
				Toggle(w, "\u25BA", "ButtonRight", NGUIWidget.Pivot.Right,  true);
				Toggle(w, "\u25B2", "ButtonLeft",  NGUIWidget.Pivot.Top,    false);
				Toggle(w, "\u258C", "ButtonMid",   NGUIWidget.Pivot.Center, false);
				Toggle(w, "\u25BC", "ButtonRight", NGUIWidget.Pivot.Bottom, false);
		...
		static void Toggle(NGUIWidget w, string text, string style, NGUIWidget.Pivot pivot, bool isHorizontal)
		{
			bool isActive = false;
		...
		static void SetPivot(NGUIWidget w, NGUIWidget.Pivot pivot, bool isHorizontal)
		{
			NGUIWidget.Pivot horizontal = GetHorizontal(w.pivot);
			NGUIWidget.Pivot vertical   = GetVertical(w.pivot);
