namespace NGUIEditor
{
	...
	public class UICameraTool : EditorWindow
	{
		...
		static public int LayerMaskField(string label, int mask, params GUILayoutOption[] options)
		{
			...
			{
				newSelected =
					EditorGUILayout.Popup(newSelected, layers.ToArray(), EditorStyles.layerMaskField, options);
			}
			else
			{
		...
		{
			bool highlight = (cam == null || Selection.activeGameObject == null)
				                 ? false
			...
			else
			{
				...
				{
					if (uic == null) uic = cam.gameObject.AddComponent<NGUICamera>();
					uic.enabled = !ev;
				}
			}
