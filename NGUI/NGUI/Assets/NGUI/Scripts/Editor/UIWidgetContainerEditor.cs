namespace NGUIEditor
{
	...
	public class UIWidgetContainerEditor : Editor
	{
		...
		public void OnSceneGUI()
		{
			...
			if (mb.GetComponent<NGUIWidget>() != null) return;
			if (mb.GetComponent<NGUIPanel>()  != null) return;

			Transform  t       = mb.transform;
			NGUIWidget[] widgets = t.GetComponentsInChildren<NGUIWidget>();
			...
			if (widgets.Length > 0)
			{
				...
				isWithinRect = mIsDragging || (e.modifiers == 0 &&
				                               NGUIEditorTools.SceneViewDistanceToRectangle(corners, e.mousePosition) ==
				                               0f);
#if !UNITY_3_5
			...
			switch (type)
			{
				...
				case EventType.MouseDown:
				{
					...
					else if (e.button == 0 && isWithinRect && corners != null &&
					         UIWidgetInspector.Raycast(corners, out mStartDrag))
					{
						mCanDrag              = true;
