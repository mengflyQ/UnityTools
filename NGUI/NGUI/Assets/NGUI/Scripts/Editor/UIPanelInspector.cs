namespace NGUIEditor
{
	...
	public class UIPanelInspector : UIRectEditor
	{
		...
		public void OnSceneGUI()
		{
			...
			NGUICamera cam = NGUICamera.FindCameraForLayer(mPanel.gameObject.layer);
		...
		if (cam == null || !cam.cachedCamera.isOrthoGraphic) return;
			...
			NGUIWidget.Pivot pivotUnderMouse =
				UIWidgetInspector.GetPivotUnderMouse(handles, e, resizable, true, ref actionUnderMouse);
		...
		protected override bool ShouldDrawProperties()
		{
			...
			if (matchingDepths > 1)
			{
				EditorGUILayout.HelpBox(matchingDepths + " panels are sharing the depth value of " + mPanel.depth,
				                        MessageType.Warning);
			}

			NGUIDrawCall.Clipping clipping = (NGUIDrawCall.Clipping) EditorGUILayout.EnumPopup("Clipping", mPanel.clipping);
			...
			if (mPanel.clipping != NGUIDrawCall.Clipping.None)
			{
				...
				if (mPanel.clipping == NGUIDrawCall.Clipping.SoftClip)
				{
					GUILayout.BeginHorizontal();
					GUILayout.Space(80f);
					Vector2 soft =
						EditorGUILayout.Vector2Field("Softness", mPanel.clipSoftness, GUILayout.MinWidth(20f));
					GUILayout.EndHorizontal();
				...
				else if (mPanel.clipping == NGUIDrawCall.Clipping.TextureMask)
				{
			...
			if (clipping != NGUIDrawCall.Clipping.None && !NGUIEditorTools.IsUniform(mPanel.transform.lossyScale))
			{
				EditorGUILayout.HelpBox("Clipped panels must have a uniform scale, or clipping won't work properly!",
				                        MessageType.Error);
			...
			if (NGUIEditorTools.DrawHeader("Advanced Options"))
			{
				...
				GUILayout.BeginHorizontal();
				NGUIPanel.RenderQueue rq =
					(NGUIPanel.RenderQueue) EditorGUILayout.EnumPopup("Render Q", mPanel.renderQueue);
				...
				if (rq != NGUIPanel.RenderQueue.Automatic)
				{
				...
				GUILayout.BeginHorizontal();
				EditorGUI.BeginDisabledGroup(mPanel.GetComponent<NGUIRoot>() != null);
				bool off = EditorGUILayout.Toggle("Offset",
				                                  mPanel.anchorOffset && mPanel.GetComponent<NGUIRoot>() == null,
				...
				{
					EditorGUILayout
					   .HelpBox("Only mark the panel as 'static' if you know FOR CERTAIN that the widgets underneath will not move, rotate, or scale. Doing this improves performance, but moving widgets around will have no effect.",
					            MessageType.Warning);
				}
		...
		void AdjustClipping(NGUIPanel p, Vector3 startLocalPos, Vector4 startCR, Vector3 worldDelta, NGUIWidget.Pivot pivot)
		{
		...
		void AdjustClipping(NGUIPanel p, Vector4 cr, int left, int top, int right, int bottom)
		{
