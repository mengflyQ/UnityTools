namespace NGUIEditor
{
	...
	public class EventDelegateDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect rect, SerializedProperty prop, GUIContent label)
		{
			...
			target =
				EditorGUI.ObjectField(lineRect, "Notify", target, typeof(MonoBehaviour), true) as MonoBehaviour;
				...
				if (ps != null)
				{
					...
					for (int i = 0; i < ps.Length; i++)
					{
						...
						if (selGO != null)
						{
							...
							List<PropertyReferenceDrawer.Entry> ents =
								PropertyReferenceDrawer.GetProperties(selGO, true, false);

							int selection;
							string[] props =
								EventDelegateEditor.GetNames(ents, NGUITools.GetFuncName(param.obj, param.field),
								                             out selection);
