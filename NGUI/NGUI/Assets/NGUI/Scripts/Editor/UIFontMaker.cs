namespace NGUIEditor
{
	...
	public class UIFontMaker : EditorWindow
	{
		...
		void MarkAsChanged()
		{
			...
			if (obj != null)
			{
				List<NGUILabel> labels = NGUIEditorTools.FindAll<NGUILabel>();
		...
		void OnGUI()
		{
			...
			{
				NGUISettings.fontData =
					EditorGUILayout.ObjectField("Font Data", NGUISettings.fontData, typeof(TextAsset), false) as
						TextAsset;
				NGUISettings.fontTexture =
					EditorGUILayout.ObjectField("Texture", NGUISettings.fontTexture, typeof(Texture2D), false,
					                            GUILayout.Width(140f)) as Texture2D;
				...
				{
					EditorGUILayout
					   .HelpBox("To create a font from a previously exported FNT file, you need to use BMFont on "       +
					            "Windows or your choice of Glyph Designer or the less expensive bmGlyph on the Mac.\n\n" +
					            "Either of these tools will create a FNT file for you that you will drag & drop into the field above.",
					            MessageType.Info);
				}
				else if (NGUISettings.fontTexture == null)
				{
					EditorGUILayout
					   .HelpBox("When exporting your font, you should get two files: the FNT, and the texture. Only one texture can be used per font.",
					            MessageType.Info);
				}
				else if (NGUISettings.atlas == null)
				{
					EditorGUILayout
					   .HelpBox("You can create a font that doesn't use a texture atlas. This will mean that the text " +
					            "labels using this font will generate an extra draw call.\n\nIf you do specify an atlas, the font's texture will be added to it automatically.",
					            MessageType.Info);
				}
			...
			else
			{
				...
				if (mType == FontType.Dynamic)
				{
					...
					{
						EditorGUILayout
						   .HelpBox("You don't have to create a NGUIFont to use dynamic fonts. You can just reference the Unity Font directly on the label.",
						            MessageType.Info);
					}

					EditorGUILayout
					   .HelpBox("Please note that dynamic fonts can't be made a part of an atlas, and using dynamic fonts will result in at least one extra draw call.",
					            MessageType.Warning);
				}
				else
				{
					...
					EditorGUI.BeginDisabledGroup(ttf == null || isBuiltIn || !FreeType.isPresent);
					{
						...
						{
							EditorGUILayout
							   .HelpBox("You can create a bitmap font by specifying a dynamic font to use as the source.",
							            MessageType.Info);
						}
						else if (isBuiltIn)
						{
							EditorGUILayout
							   .HelpBox("You chose an embedded font. You can't create a bitmap font from an embedded resource.",
							            MessageType.Warning);
						}
						else if (NGUISettings.atlas == null)
						{
