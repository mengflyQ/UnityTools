...
static public class NGUIEditorTools
{
	...
	static public void DrawSprite (Texture2D tex, Rect rect, NGUISpriteData sprite, Color color)
	{
	...
	static public void DrawSprite (Texture2D tex, Rect drawRect, NGUISpriteData sprite, Color color, Material mat)
	{
				...
				if (newName != spriteName)
				{
					...
					if (GUILayout.Button("Rename", GUILayout.Width(60f)))
					{
						...
						if (sprite != null)
						{
							...
							var sprites = FindAll<NGUISprite>();
	...
	static public bool IsUniform (GameObject go)
	{
		...
		if (go.GetComponent<NGUIWidget>() != null)
		{
	...
	static public void FixUniform (GameObject go)
	{
		...
		while (t != null && t.gameObject.GetComponent<NGUIRoot>() == null)
		{
	...
	static public List<NGUIWidget> SceneViewRaycast (Vector2 mousePos)
	{
		...
		list.Sort(NGUIWidget.FullCompareFunc);
		return list;
	}
	...
	{
		GameObject go = null;
		List<NGUIWidget> widgets = SceneViewRaycast(pos);
...
#endif
			NGUIWidget.showHandlesWithMoveTool && !NGUISettings.showTransformHandles;
#endif
	}
	...
	{
		if (atlas == null) return;
		List<NGUITexture> uits = FindAll<NGUITexture>();

		if (uits.Count > 0)
		{
			NGUIWidget selectedTex = (UIWidgetInspector.instance != null && UIWidgetInspector.instance.target != null) ?
				UIWidgetInspector.instance.target as NGUITexture : null;

			// Determine the object instance ID of the NGUISprite class
			int spriteID = GetClassID<NGUISprite>();
	...
	{
		List<NGUIWidget> widgets = NGUIEditorTools.SceneViewRaycast(screenPos);
		List<UIWidgetContainer> containers = new List<UIWidgetContainer>();
		List<MenuEntry> entries = new List<MenuEntry>();
		List<NGUIPanel> panels = new List<NGUIPanel>();

		bool divider = false;
		NGUIWidget topWidget = null;
		NGUIPanel topPanel = null;
