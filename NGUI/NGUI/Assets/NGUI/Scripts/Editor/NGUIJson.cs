...
// http://techblog.procurios.nl/k/618/news/view/14605/14863/How-do-I-write-my-own-parser-for-JSON.html
	...
	public class NGUIJson
	{
		...
		static void LoadSpriteData(INGUIAtlas atlas, Hashtable decodedHash)
		{
			...
			List<NGUISpriteData> oldSprites = atlas.spriteList;
			atlas.spriteList = new List<NGUISpriteData>();

			Hashtable frames = (Hashtable) decodedHash["frames"];

			foreach (System.Collections.DictionaryEntry item in frames)
			{
				NGUISpriteData newSprite = new NGUISpriteData();
				newSprite.name = item.Key.ToString();

				bool exists = false;

				// Check to see if this sprite exists
				foreach (NGUISpriteData oldSprite in oldSprites)
				{
		...
		static int CompareSprites(NGUISpriteData a, NGUISpriteData b)
		{
