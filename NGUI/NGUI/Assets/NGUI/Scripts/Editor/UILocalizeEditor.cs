namespace NGUIEditor
{
	...
	[CustomEditor(typeof(NGUILocalize), true)]
			...
			if (isPresent)
			{
				if (NGUIEditorTools.DrawHeader("Preview"))
				{
					...
					if (Localization.dictionary.TryGetValue(myKey, out values))
					{
						...
						{
							EditorGUILayout
							   .HelpBox("Number of keys doesn't match the number of values! Did you modify the dictionaries by hand at some point?",
							            MessageType.Error);
						}
						else
						{
							for (int i = 0; i < keys.Length; ++i)
							{
								...
								if (GUILayout.Button(values[i], NGUIEditorTools.textArea, GUILayout.MinWidth(80f),
								                     GUILayout.MaxWidth(Screen.width - 110f)))
								{
									(target as NGUILocalize).value = values[i];
