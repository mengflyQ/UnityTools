namespace NGUIEditor
{
	...
	public class NGUISettings
	{
		...
		static public INGUIAtlas atlas
		{
			...
			set { Set("NGUI Atlas", value as Object); }
		}

		static public NGUISpriteData GetSprite(string spriteName)
		{
		...
		static public NGUIWidget.Pivot pivot
		{
			get { return GetEnum("NGUI Pivot", NGUIWidget.Pivot.Center); }
		...
		static public NGUILabel.Overflow overflowStyle
		{
			get { return GetEnum("NGUI Overflow", NGUILabel.Overflow.ShrinkContent); }
		...
		static public string defaultPathToFreeType
		{
			get
			{
				...
				if (platform      == RuntimePlatform.WindowsEditor) path = System.IO.Path.Combine(path, "FreeType.dll");
					...
					path = System.IO.Path.Combine(path, "FreeType.dylib");
				return path.Replace('\\', '/');
			}
		}
		...
		static public NGUIWidget AddWidget(GameObject go)
		{
			NGUIWidget w = NGUITools.AddWidget<NGUIWidget>(go);
			w.name   = "Container";
			w.pivot  = pivot;
			w.width  = 100;
			w.height = 100;
		...
		static public NGUITexture AddTexture(GameObject go)
		{
			NGUITexture w = NGUITools.AddWidget<NGUITexture>(go);
			w.name        = "Texture";
			w.pivot       = pivot;
		...
		static public NGUI2DSprite Add2DSprite(GameObject go)
		{
			NGUI2DSprite w = NGUITools.AddWidget<NGUI2DSprite>(go);
			w.name     = "2D Sprite";
			w.pivot    = pivot;
			w.sprite2D = sprite2D;
			w.width    = 100;
		...
		static public NGUISprite AddSprite(GameObject go)
		{
			NGUISprite w = NGUITools.AddWidget<NGUISprite>(go);
			w.name       = "Sprite";
			w.atlas      = atlas;
			w.spriteName = selectedSprite;

			var sp                                 = w.GetAtlasSprite();
			if (sp != null && sp.hasBorder) w.type = NGUISprite.Type.Sliced;
		...
		static public NGUISpriteCollection AddSpriteCollection(GameObject go)
		{
			NGUISpriteCollection w = NGUITools.AddWidget<NGUISpriteCollection>(go);
		...
		static public NGUILabel AddLabel(GameObject go)
		{
			NGUILabel w = NGUITools.AddWidget<NGUILabel>(go);
			w.name           = "Label";
		...
		static public NGUIPanel AddPanel(GameObject go)
		{
			if (go == null) return null;
			int     depth = NGUIPanel.nextUnusedDepth;
			NGUIPanel panel = NGUITools.AddChild<NGUIPanel>(go);
			panel.depth = depth;
			return panel;
		}
		...
		static public void CopyWidget(NGUIWidget widget)
		{
			...
			if (widget is NGUISprite) CopySprite(widget as NGUISprite);
			else if (widget is NGUILabel) CopyLabel(widget as NGUILabel);
		}
		...
		static public void PasteWidget(NGUIWidget widget, bool fully)
		{
			widget.color = GetColor("Widget Color", widget.color);
			widget.pivot = GetEnum<NGUIWidget.Pivot>("Widget Pivot", widget.pivot);
			...
			if (widget is NGUISprite) PasteSprite(widget as NGUISprite, fully);
			else if (widget is NGUILabel) PasteLabel(widget as NGUILabel, fully);
		}
		...
		static void CopySprite(NGUISprite sp)
		{
		...
		static void CopyLabel(NGUILabel lbl)
		{
		...
		static void PasteSprite(NGUISprite sp, bool fully)
		{
			sp.spriteName    = GetString("Sprite", sp.spriteName);
			sp.type          = GetEnum<NGUISprite.Type>("Sprite Type", sp.type);
			sp.leftType      = GetEnum<NGUISprite.AdvancedType>("Left Type",   NGUISprite.AdvancedType.Sliced);
			sp.rightType     = GetEnum<NGUISprite.AdvancedType>("Right Type",  NGUISprite.AdvancedType.Sliced);
			sp.topType       = GetEnum<NGUISprite.AdvancedType>("Top Type",    NGUISprite.AdvancedType.Sliced);
			sp.bottomType    = GetEnum<NGUISprite.AdvancedType>("Bottom Type", NGUISprite.AdvancedType.Sliced);
			sp.centerType    = GetEnum<NGUISprite.AdvancedType>("Center Type", NGUISprite.AdvancedType.Sliced);
			sp.fillAmount    = GetFloat("Fill", sp.fillAmount);
			sp.fillDirection = GetEnum<NGUISprite.FillDirection>("FDir", sp.fillDirection);
		...
		static void PasteLabel(NGUILabel lbl, bool fully)
		{
			if (fully)
			{
			...
			lbl.overflowMethod  = GetEnum<NGUILabel.Overflow>("Overflow", lbl.overflowMethod);
			...
			lbl.effectStyle     = GetEnum<NGUILabel.Effect>("Effect", lbl.effectStyle);
