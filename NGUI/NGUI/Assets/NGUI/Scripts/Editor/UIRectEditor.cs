namespace NGUIEditor
{
	...
	[CustomEditor(typeof(NGUIRect), true)]
	public class UIRectEditor : Editor
	{
		...
		static protected string[] HorizontalList = new string[]
		                                           {
			                                           "Target's Left", "Target's Center", "Target's Right", "Custom",
			                                           "Set to Current Position"
		                                           };

		static protected string[] VerticalList = new string[]
		                                         {
			                                         "Target's Bottom", "Target's Center", "Target's Top", "Custom",
			                                         "Set to Current Position"
		                                         };
		...
		static protected bool IsCommon(float relative)
		{
			return (relative == 0f || relative == 0.5f || relative == 1f);
		}

		/// <summary>
		/// Returns 'true' if the specified serialized property reference is a NGUIRect.
		/// </summary>
		...
		static protected NGUIRect GetRect(SerializedProperty sp)
		{
			Transform target = sp.objectReferenceValue as Transform;
			if (target == null) return null;
			return target.GetComponent<NGUIRect>();
		}
		...
		{
			NGUIRect rect = target as NGUIRect;
		...
		{
			if (!NGUISettings.unifiedTransform) DrawAnchorTransform();
		}
		...
		public void DrawAnchorTransform()
		{
			if (NGUIEditorTools.DrawHeader("Anchors"))
			{
				...
				EditorGUI.BeginDisabledGroup(!((target as NGUIRect).canBeAnchored));
				...
				if (mAnchorType == AnchorType.None && type != AnchorType.None)
				{
					if (type == AnchorType.Unified)
					{
						...
						{
							NGUIRect rect   = target as NGUIRect;
							NGUIRect parent = NGUITools.FindInParents<NGUIRect>(rect.cachedTransform.parent);
		...
		protected void DrawAnchor(int index, bool targetSelection)
		{
			...
			NGUIRect myRect = serializedObject.targetObject as NGUIRect;
			...
			NGUIRect targetRect     = GetRect(tar);
		...
		protected void UpdateAnchors(bool resetRelative)
		{
			...
			{
				NGUIRect rect = objs[i] as NGUIRect;
		...
		static public void UpdateHorizontalAnchor(NGUIRect r, NGUIRect.AnchorPoint anchor, bool resetRelative)
		{
			// Update the target
			if (anchor.target == null) return;

			// Update the rect
			anchor.rect = anchor.target.GetComponent<NGUIRect>();
		...
		static public void UpdateVerticalAnchor(NGUIRect r, NGUIRect.AnchorPoint anchor, bool resetRelative)
		{
			// Update the target
			if (anchor.target == null) return;
