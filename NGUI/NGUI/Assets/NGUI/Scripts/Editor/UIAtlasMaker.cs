namespace NGUIEditor
{
	...
	public class UIAtlasMaker : EditorWindow
	{
		...
		public class SpriteEntry : NGUISpriteData
		{
			...
			public void SetTexture(Color32[] newPixels, int newWidth, int newHeight)
			{
				...
				tempGO = EditorUtility.CreateGameObjectWithHideFlags(name, HideFlags.HideAndDontSave,
				                                                     typeof(MeshRenderer));
		...
		static public NGUISpriteData AddSprite(List<NGUISpriteData> sprites, SpriteEntry se)
		{
			// See if this sprite already exists
			foreach (NGUISpriteData sp in sprites)
			{
				if (sp.name == se.name)
				{
					sp.CopyFrom(se);
			...
			NGUISpriteData sprite = new NGUISpriteData();
			sprite.CopyFrom(se);
			sprites.Add(sprite);
			return sprite;
		}
		...
		static public void ReplaceSprites(INGUIAtlas atlas, List<SpriteEntry> sprites)
		{
			...
			List<NGUISpriteData> kept       = new List<NGUISpriteData>();
		...
		{
			if (atlas == null || atlas.texture == null) return null;
			NGUISpriteData sd = atlas.GetSprite(spriteName);
			if (sd == null) return null;
		...
		static SpriteEntry ExtractSprite(NGUISpriteData es, Texture2D tex)
		{
		...
		static SpriteEntry ExtractSprite(NGUISpriteData es, Color32[] oldPixels, int oldWidth, int oldHeight)
		{
		...
		static List<NGUISpriteData> spriteList
		{
			get
			{
				var atlas = NGUISettings.atlas;
		...
		static public void ExtractSprites(INGUIAtlas atlas, List<SpriteEntry> finalSprites)
		{
			...
			if (tex != null)
			{
				...
				if (sprites != null)
				{
					...
					foreach (NGUISpriteData es in sprites)
					{
		...
		static public bool UpdateTexture(INGUIAtlas atlas, List<SpriteEntry> sprites)
		{
				...
				if (newTexture)
				{
					...
					else
					{
						...
						if (mat == null)
						{
							...
							var shader = Shader.Find(NGUISettings.atlasPMA
			...
			else
			{
				...
				//	"Keep large sprites outside the atlas (use NGUITexture), and/or use multiple atlases instead.");
		...
		void OnGUI()
		{
			...
			GUILayout.BeginHorizontal();
			NGUISettings.atlasPadding =
				Mathf.Clamp(EditorGUILayout.IntField("Padding", NGUISettings.atlasPadding, GUILayout.Width(100f)), 0,
				            8);
			...
			{
				GUILayout.BeginHorizontal();
				NGUISettings.atlasPMA =
					EditorGUILayout.Toggle("PMA Shader", NGUISettings.atlasPMA, GUILayout.Width(100f));
...
#if !UNITY_5_6
			GUILayout.BeginHorizontal();
			NGUISettings.unityPacking =
				EditorGUILayout.Toggle("Unity Packer", NGUISettings.unityPacking, GUILayout.Width(100f));
			...
			NGUISettings.trueColorAtlas =
				EditorGUILayout.Toggle("Truecolor", NGUISettings.trueColorAtlas, GUILayout.Width(100f));
			GUILayout.Label("force ARGB32 textures", GUILayout.MinWidth(70f));
			GUILayout.EndHorizontal();

			GUILayout.BeginHorizontal();
			NGUISettings.autoUpgradeSprites =
				EditorGUILayout.Toggle("Auto-upgrade", NGUISettings.autoUpgradeSprites, GUILayout.Width(100f));
			...
			{
				GUILayout.BeginHorizontal();
				NGUISettings.forceSquareAtlas =
					EditorGUILayout.Toggle("Force Square", NGUISettings.forceSquareAtlas, GUILayout.Width(100f));
		...
		GUILayout.EndHorizontal();
			...
			{
				EditorGUILayout
				   .HelpBox("You can create a new atlas by selecting one or more textures in the Project View window, then clicking \"Create\".",
				            MessageType.Info);
				...
				if (create)
				{
					var path = EditorUtility.SaveFilePanelInProject("Save As", "New Atlas.asset", "asset",
					                                                "Save atlas as...", NGUISettings.currentPath);
			...
			if (spriteList.Count > 0)
			{
				NGUIEditorTools.DrawHeader("Sprites", true);
				{
					...
					foreach (KeyValuePair<string, int> iter in spriteList)
					{
						...
						GUILayout.Space(-1f);
						bool highlight = (NGUISettings.selectedSprite == iter.Key);
			...
			if (NGUISettings.atlas != null && textures.Count == 0)
				EditorGUILayout
				   .HelpBox("You can reveal more options by selecting one or more textures in the Project View window.",
				            MessageType.Info);
