namespace NGUIEditor
{
    ...
    public class UIPrefabTool : EditorWindow
    {
        ...
        public void RegenerateTexture(GameObject prefab, NGUISnapshotPoint point)
        {
            for (int i = 0; i < mItems.Size; ++i)
            {
        ...
        static NGUISnapshotPoint GetSnapshotPoint(Transform t)
        {
            NGUISnapshotPoint point = t.GetComponent<NGUISnapshotPoint>();
        ...
        void GeneratePreview(Item item, NGUISnapshotPoint point)
        {
		...
		GameObject camGO =
 EditorUtility.CreateGameObjectWithHideFlags("Preview Camera", HideFlags.HideAndDontSave, typeof(Camera));
		...
		if (point == null) point = child.GetComponentInChildren<NGUISnapshotPoint>();

		// If there is a NGUIRect present (widgets or panels) then it's an NGUI object
        ...
        static bool SetupPreviewForUI(Camera cam, GameObject root, GameObject child, NGUISnapshotPoint point)
        {
            if (child.GetComponentInChildren<NGUIRect>() == null) return false;

            if (child.GetComponent<NGUIPanel>() == null)
                root.AddComponent<NGUIPanel>();
            ...
            if (point != null) SetupSnapshotCamera(child, cam, point);
                ...
                SetupSnapshotCamera(child, cam, objSize, Mathf.RoundToInt(Mathf.Max(size.x, size.y)),
                                    -100f, 100f);
        ...
        static bool SetupPreviewFor3D(Camera cam, GameObject root, GameObject child, NGUISnapshotPoint point)
        {
        ...
        static void SetupSnapshotCamera(GameObject go, Camera cam, NGUISnapshotPoint point)
        {
        ...
        static void SetupSnapshotCamera(GameObject go, Camera cam, float objectSize, float orthoSize, float near,
                                        float far)
        {
        ...
        void OnGUI()
        {
            ...
            {
                string after =
                    EditorGUILayout.TextField("", searchFilter, "SearchTextField", GUILayout.Width(Screen.width - 20f));
            ...
            for (int i = 0; i < mItems.Size;)
            {
                ...
                if (mItems.Buffer[i] != selection)
                {
                    if (string.IsNullOrEmpty(searchFilter) ||
                        mItems.Buffer[i].prefab.name
                              .IndexOf(searchFilter, System.StringComparison.CurrentCultureIgnoreCase) != -1)
                        indices.Add(i);
                }
            ...
            mPos = GUILayout.BeginScrollView(mPos);
            {
                ...
                for (int i = 0; i < indices.Size; ++i)
                {
                    ...
                    inner.yMax -= 2f;
                    rect.yMax -=
                        1f; // Button seems to be mis-shaped. It's height is larger than its width by a single pixel.
                    ...
                    if (GUI.Button(rect, mContent, "Button"))
                    {
                        ...
                        {
                            string path =
                                EditorUtility.OpenFilePanel("Add a prefab", NGUISettings.currentPath, "prefab");
