...
[CustomEditor(typeof(UIButton))]
	...
	public class UIButtonEditor : UIButtonColorEditor
	{
		...
		{
			if (!(target is NGUIButtonLongPress))
			{
				SerializedProperty idc = serializedObject.FindProperty("IsDoubleClick");
				GUILayout.BeginHorizontal();
				idc.boolValue = EditorGUILayout.Toggle("Is Double Click", idc.boolValue);
			...
			{
				var sprite = btn.tweenTarget.GetComponent<NGUISprite>();
				var s2d    = btn.tweenTarget.GetComponent<NGUI2DSprite>();
				...
				if (sprite != null)
				{
					if (NGUIEditorTools.DrawHeader("Sprites", "Sprites", false, true))
					{
						...
						EditorGUI.BeginDisabledGroup(serializedObject.isEditingMultipleObjects);
						{
							...
							NGUIEditorTools.DrawSpriteField("Hover", serializedObject, atlas,
							                                serializedObject.FindProperty("hoverSprite"), true);
							NGUIEditorTools.DrawSpriteField("Pressed", serializedObject, atlas,
							                                serializedObject.FindProperty("pressedSprite"), true);
							NGUIEditorTools.DrawSpriteField("Disabled", serializedObject, atlas,
							                                serializedObject.FindProperty("disabledSprite"), true);
						}
