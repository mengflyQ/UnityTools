	...
	public class NGUIFontInspector : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.HelpBox("Dynamic fonts require Unity 4.0 or higher.", MessageType.Error);
				...
				Material mat =
					EditorGUILayout.ObjectField("Material", mFont.material, typeof(Material), false) as Material;
			...
			else
			{
				...
				{
					TextAsset data =
						EditorGUILayout.ObjectField("Import Data", null, typeof(TextAsset), false) as TextAsset;
			...
			if (!mFont.isDynamic && mFont.bmFont.isValid)
			{
				if (mFont.atlas != null)
				{
					if (NGUIEditorTools.DrawHeader("Symbols and Emoticons"))
					{
						...
						{
							EditorGUILayout
							   .HelpBox("Want to add an emoticon to your font? In the field above type ':)', choose a sprite, then hit the Add button.",
							            MessageType.Info);
						}
			...
			if (mFont.bmFont != null && mFont.bmFont.isValid)
			{
				if (NGUIEditorTools.DrawHeader("Modify"))
				{
					...
					NGUISpriteData sd = mFont.sprite;
					...
					NGUISettings.foregroundColor =
						EditorGUILayout.ColorField("Foreground", NGUISettings.foregroundColor);
					...
					if (disable)
					{
						GUILayout.Space(3f);
						EditorGUILayout
						   .HelpBox("The sprite used by this font has been trimmed and is not suitable for modification. " +
