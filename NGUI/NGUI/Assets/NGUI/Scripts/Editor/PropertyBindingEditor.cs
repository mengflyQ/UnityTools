		...
		public override void OnInspectorGUI()
		{
			...
			if (!serializedObject.isEditingMultipleObjects)
			{
				    ...
				    pb.source.GetPropertyType() != pb.target.GetPropertyType())
				{
					...
					{
						EditorGUILayout
						   .HelpBox("Bi-Directional updates require both Source and Target to reference values of the same type.",
						            MessageType.Error);
					}
					else if (pb.direction == PropertyBinding.Direction.SourceUpdatesTarget)
					{
						...
						{
							EditorGUILayout
							   .HelpBox("Unable to convert " + pb.source.GetPropertyType() + " to " + pb.target.GetPropertyType(),
							            MessageType.Error);
						}
					}
					...
					{
						EditorGUILayout
						   .HelpBox("Unable to convert " + pb.target.GetPropertyType() + " to " + pb.source.GetPropertyType(),
						            MessageType.Error);
					}
				}
			}
