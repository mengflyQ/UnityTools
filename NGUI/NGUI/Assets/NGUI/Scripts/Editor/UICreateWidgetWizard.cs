...
namespace NGUIEditor
{
	...
	public class UICreateWidgetWizard : EditorWindow
	{
		...
		static public bool ShouldCreate(GameObject go, bool isValid)
		{
			...
			GameObject sel =
				EditorGUILayout.ObjectField(go, typeof(GameObject), true, GUILayout.Width(140f)) as GameObject;
		...
		void CreateSprite(GameObject go)
		{
			...
			{
				NGUIEditorTools.DrawSpriteField("Sprite", "Sprite that will be created", NGUISettings.atlas,
				                                NGUISettings.selectedSprite, OnSprite, GUILayout.Width(120f));

				if (!string.IsNullOrEmpty(NGUISettings.selectedSprite))
				{
					GUILayout.BeginHorizontal();
					NGUISettings.pivot =
						(NGUIWidget.Pivot) EditorGUILayout.EnumPopup("Pivot", NGUISettings.pivot, GUILayout.Width(200f));
					GUILayout.Space(20f);
		...
		void CreateButton(GameObject go)
		{
			if (NGUISettings.atlas != null)
			{
				NGUIEditorTools.DrawSpriteField("Background", "Sliced Sprite for the background", NGUISettings.atlas,
				                                mButton, OnButton, GUILayout.Width(120f));
			}
		...
		void CreateImageButton(GameObject go)
		{
			if (NGUISettings.atlas != null)
			{
				...
				NGUIEditorTools.DrawSpriteField("Hover", "Hover state sprite", NGUISettings.atlas, mImage1, OnImage1,
				                                GUILayout.Width(120f));
				NGUIEditorTools.DrawSpriteField("Pressed", "Pressed state sprite", NGUISettings.atlas, mImage2,
				                                OnImage2, GUILayout.Width(120f));
				NGUIEditorTools.DrawSpriteField("Disabled", "Disabled state sprite", NGUISettings.atlas, mImage3,
				                                OnImage3, GUILayout.Width(120f));
			}
			...
			if (ShouldCreate(go, NGUISettings.atlas != null))
			{
				...
				NGUISpriteData sp     = NGUISettings.GetSprite(mImage0);
				NGUISprite     sprite = NGUITools.AddWidget<NGUISprite>(go);
				sprite.type       = sp.hasBorder ? NGUISprite.Type.Sliced : NGUISprite.Type.Simple;
		...
		void CreateToggle(GameObject go)
		{
			if (NGUISettings.atlas != null)
			{
				NGUIEditorTools.DrawSpriteField("Background", "Sprite used for the background", NGUISettings.atlas,
				                                mCheckBG, OnCheckBG, GUILayout.Width(120f));
				NGUIEditorTools.DrawSpriteField("Checkmark", "Sprite used for the checkmark", NGUISettings.atlas,
				                                mCheck, OnCheck, GUILayout.Width(120f));
			}
		...
		void CreateScrollBar(GameObject go)
		{
			...
			{
				NGUIEditorTools.DrawSpriteField("Background", "Sprite used for the background", NGUISettings.atlas,
				                                mScrollBG, OnScrollBG, GUILayout.Width(120f));
				NGUIEditorTools.DrawSpriteField("Foreground", "Sprite used for the foreground (thumb)",
				                                NGUISettings.atlas, mScrollFG, OnScrollFG, GUILayout.Width(120f));
		...
		void CreateSlider(GameObject go, bool slider)
		{
			if (NGUISettings.atlas != null)
			{
				NGUIEditorTools.DrawSpriteField("Empty", "Sprite for the background (empty bar)", NGUISettings.atlas,
				                                mSliderBG, OnSliderBG, GUILayout.Width(120f));
				NGUIEditorTools.DrawSpriteField("Full", "Sprite for the foreground (full bar)", NGUISettings.atlas,
				                                mSliderFG, OnSliderFG, GUILayout.Width(120f));

				if (slider)
				{
					NGUIEditorTools.DrawSpriteField("Thumb", "Sprite for the thumb indicator", NGUISettings.atlas,
					                                mSliderTB, OnSliderTB, GUILayout.Width(120f));
				}
			}
			...
			if (ShouldCreate(go, NGUISettings.atlas != null))
			{
				...
				var back = NGUITools.AddWidget<NGUISprite>(go);

				back.type                    = bgs.hasBorder ? NGUISprite.Type.Sliced : NGUISprite.Type.Simple;
				...
				// Foreground sprite
				NGUISpriteData fgs   = NGUISettings.GetSprite(mSliderFG);
				NGUISprite     front = NGUITools.AddWidget<NGUISprite>(go);
				front.type                    = fgs.hasBorder ? NGUISprite.Type.Sliced : NGUISprite.Type.Simple;
				...
				if (slider)
				{
					NGUISpriteData tbs = NGUISettings.GetSprite(mSliderTB);
					NGUISprite     thb = NGUITools.AddWidget<NGUISprite>(go);

					thb.type                    = tbs.hasBorder ? NGUISprite.Type.Sliced : NGUISprite.Type.Simple;
					thb.name                    = "Thumb";
		...
		void CreateInput(GameObject go)
		{
			...
			{
				NGUIEditorTools.DrawSpriteField("Background", "Sliced Sprite for the background", NGUISettings.atlas,
				                                mInputBG, OnInputBG, GUILayout.Width(120f));
			}
			...
			if (ShouldCreate(go, NGUISettings.atlas != null && NGUISettings.ambigiousFont != null))
			{
				...
				NGUIInputField input = go.AddComponent<NGUIInputField>();
				input.label = lbl;
		...
		void CreatePopup(GameObject go, bool isDropDown)
		{
			if (NGUISettings.atlas != null)
			{
				NGUIEditorTools.DrawSpriteField("Foreground", "Foreground sprite (shown on the button)",
				                                NGUISettings.atlas, mListFG, OnListFG, GUILayout.Width(120f));
				NGUIEditorTools.DrawSpriteField("Background", "Background sprite (envelops the options)",
				                                NGUISettings.atlas, mListBG, OnListBG, GUILayout.Width(120f));
				NGUIEditorTools.DrawSpriteField("Highlight", "Sprite used to highlight the selected option",
				                                NGUISettings.atlas, mListHL, OnListHL, GUILayout.Width(120f));
			}
			...
			if (ShouldCreate(go, NGUISettings.atlas != null && NGUISettings.ambigiousFont != null))
			{
				...
				NGUISpriteData sphl = NGUISettings.GetSprite(mListHL);
				NGUISpriteData spfg = NGUISettings.GetSprite(mListFG);
				...
				// Background sprite
				NGUISprite sprite = NGUITools.AddSprite(go, NGUISettings.atlas as INGUIAtlas, mListFG);
				sprite.depth                   = depth;
				sprite.atlas                   = NGUISettings.atlas;
	...
	UILabelInspector.FontType mType = UILabelInspector.FontType.Unity;
		...
		void OnGUI()
		{
			...
			if (!mLoaded)
			{
				...
				mType = (font != null) ? UILabelInspector.FontType.NGUI
					        : UILabelInspector.FontType.Unity;
#else
			...
			else
			{
				...
				{
					NGUISettings.ambigiousFont =
						EditorGUILayout.ObjectField(NGUISettings.ambigiousFont, typeof(Font), false,
...
#else
			NGUISettings.ambigiousFont =
 EditorGUILayout.ObjectField(NGUISettings.ambigiousFont, typeof(UIFont), false, GUILayout.Width(140f));
#endif
				...
				EditorGUILayout
				   .HelpBox("Widget Tool has become far less useful with NGUI 3.0.6. Search the Project view for 'Control', then simply drag & drop one of them into your Scene View.",
				            MessageType.Warning);
			}
		}
	}
