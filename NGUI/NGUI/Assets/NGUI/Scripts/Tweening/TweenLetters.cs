namespace NGUI
{
    ...
    public class TweenLetters : UITweener
    {
        ...
        public class AnimationProperties
        {
            ...
            [Range(0f, 1f)] public float overlap = 0.5f;
        ...
        void OnPostFill(NGUIWidget widget, int bufferOffset, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
        {
            if (verts == null) return;
            var vertexCount = verts.Size;
		...
		try {
            ...
            for (int q = 0; q < quads; ++q)
            {
                for (int i = 0; i < characterCount; ++i)
                {
                    ...
                    {
#if UNITY_EDITOR
                        Debug.LogError("TweenLetters encountered an unhandled case trying to modify a vertex " +
                                       firstVert +
                                       ". Vertex Count: " + vertexCount + " Pass: " + q + "\nText: " + pt);
#endif
                        continue;
                    }

                    letterStart = mLetter[letter].start;
                    t = Mathf.Clamp(timeIntoAnimation - letterStart, 0f, mLetter[letter].duration) /
                        mLetter[letter].duration;
				...
				lerpAlpha = LerpUnclamped(mCurrent.alpha, 1f, t);
                    ...
                    {
                        vert = verts.Buffer[iv];
                        vert -= letterCenter;
                        vert = mtx.MultiplyPoint3x4(vert);
                        vert += letterCenter;
                        verts.Buffer[iv] = vert;
	...
	static float LerpUnclamped (float a, float b, float f) { return a + (b - a) * f; }
        ...
        void SetLetterOrder(int letterCount)
        {
            ...
            for (int i = 0; i < letterCount; ++i)
            {
                ...
                mLetter[current].offset = new Vector2(Random.Range(-mCurrent.offsetRange.x, mCurrent.offsetRange.x),
                                                      Random.Range(-mCurrent.offsetRange.y, mCurrent.offsetRange.y));
            }
        ...
        void GetLetterDuration(int letterCount)
        {
            ...
            else
            {
                ...
                float letterDuration = ScaleRange(lengthPerLetter, totalDuration + lengthPerLetter * mCurrent.overlap,
                                                  duration);
        ...
        static Vector3 GetCenter(BetterList<Vector3> verts, int firstVert, int length)
        {
            Vector3 center = verts.Buffer[firstVert];
            for (int v = firstVert + 1; v < firstVert + length; ++v) center += verts.Buffer[v];
