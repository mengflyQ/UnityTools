...
/// </summary>
/// 
[ RequireComponent ( typeof ( NGUI.NGUIPanel ), typeof ( NGUI.UIScrollView ) ) ]
[ExecuteInEditMode]
public class IPCycler : MonoBehaviour {
	...
	NGUI.UIScrollView	 		_draggablePanel;
	[HideInInspector]
	public IPDragScrollView	dragPanelContents;
	[HideInInspector]
	public IPUserInteraction	userInteraction;
	NGUI.NGUIPanel 				_panel;
	NGUI.UICenterOnChild			_uiCenterOnChild;
	...
	public void Init()
	{
		...
		_draggablePanel = gameObject.GetComponent ( typeof ( NGUI.UIScrollView ) ) as NGUI.UIScrollView;
		_panel = gameObject.GetComponent ( typeof ( NGUI.NGUIPanel ) ) as NGUI.NGUIPanel;
		
		//Make sure the parent is active before getting collider
		NGUI.NGUITools.SetActive ( transform.parent.gameObject, true ); 
		...
		_uiCenterOnChild = gameObject.AddComponent ( typeof ( NGUI.UICenterOnChild ) ) as NGUI.UICenterOnChild;
		...
		if ( dragScale != 0f )
		{
			_draggablePanel.movement = _isHorizontal ? NGUI.UIScrollView.Movement.Horizontal : NGUI.UIScrollView.Movement.Vertical;
		}
	...
	{
		Vector3 onClickTouchPosInWorld = NGUI.NGUICamera.currentCamera.ScreenToWorldPoint ( new Vector3 ( pos.x, pos.y, 0f ) );
	...
	/// </summary>
	public T[] MakeWidgets < T >  () where T : NGUI.NGUIWidget
	{
		T[] ret = new T[ NbOfTransforms ];
		
		for ( int i = 0; i < NbOfTransforms; i++ )
		{
			ret[i] = NGUI.NGUITools.AddWidget < T > ( _cycledTransforms[i].gameObject );
		}
